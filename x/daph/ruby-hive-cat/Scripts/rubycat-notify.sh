#!/bin/bash
# RubyHiveCat notify script for Claude Code hooks.
...
if tmux list-sessions &>/dev/null; then
    ...
    if [ -n "$TARGET" ]; then
        ...
        TERMINAL_APP=$(defaults read com.ruby.rubyhivecat terminalApp 2>/dev/null || echo "Alacritty")
...
if pgrep -x RubyHiveCat > /dev/null 2>&1; then
    open -g "rubycat://notify?target=${TARGET_ENCODED}"
fi
