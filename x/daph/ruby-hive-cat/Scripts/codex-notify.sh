#!/bin/bash
# RubyHiveCat notify script for OpenAI Codex CLI
...
# Only notify if RubyHiveCat is already running (don't relaunch after quit)
if pgrep -x RubyHiveCat > /dev/null 2>&1; then
    open -g "rubycat://notify?target=${TARGET_ENCODED}&title=Codex+ready"
fi
