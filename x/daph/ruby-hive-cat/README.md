...
- Integrates with Claude Code hooks via `rubycat://` URL scheme
...
```bash
cd x/daph/ruby-hive-cat
./Scripts/cli.sh install
```

This builds the app, installs it to `/Applications/`, sets up the notify script, and adds the `rubycat` CLI to your PATH.
...
{
  "hooks": {
    "Stop": [{"hooks": [{"type": "command", "command": "~/.claude/rubycat-notify.sh"}]}],
    "Notification": [{"matcher": "permission_prompt", "hooks": [{"type": "command", "command": "~/.claude/rubycat-notify.sh"}]}]
  }
}
```
...
After install, the `rubycat` command is available:

```
rubycat install     # First-time setup
rubycat update      # Rebuild from source, replace app, restart
rubycat start       # Launch the app
rubycat stop        # Quit the app
rubycat uninstall   # Remove everything
```

## URL Scheme

The app responds to `rubycat://` URLs:

- `rubycat://notify?target=SESSION%3AWINDOW.PANE&title=MESSAGE` - Alert the user, store tmux target for click action
- `rubycat://reset` - Return cat to normal idle state
...
```
Scripts/
├── cli.sh                 # rubycat CLI (install/update/start/stop/uninstall)
├── bundle-app.sh          # Build .app bundle from Swift source
├── rubycat-notify.sh      # Notify script installed to ~/.claude/
└── codex-notify.sh        # Notify script for OpenAI Codex

RubyHiveCat/
├── App/
│   ├── RubyHiveCatApp.swift       # App entry point
...
Claude Code hook fires
        │
        ▼
open "rubycat://notify?target=session%3Awindow.pane&title=..."
        │
        ▼
...
This prevents command injection via malicious `rubycat://` URLs.
