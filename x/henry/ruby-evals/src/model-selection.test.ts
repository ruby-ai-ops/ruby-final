...
describe("parseAgentSpec", () => {
  ...
  it("parses a model override without an effort", () => {
    const res = parseAgentSpec("ruby#xai/grok-4.6")
    expect(res.isOk).toBe(true)
    if (!res.isOk) return
    expect(res.value.agentId).toBe("ruby")
  ...
  it("parses a model override with an effort", () => {
    const res = parseAgentSpec("ruby#anthropic/claude-sonnet-4-6@high")
    expect(res.isOk).toBe(true)
    if (!res.isOk) return
    expect(res.value.label).toBe("ruby#anthropic/claude-sonnet-4-6@high")
    expect(res.value.modelSelection).toEqual({
  ...
  it("keeps slashes in the model id", () => {
    const res = parseAgentSpec(
      "ruby#fireworks/accounts/fireworks/models/glm-5p3-flash@light"
    )
    expect(res.isOk).toBe(true)
    if (!res.isOk) return
    expect(res.value.modelSelection).toEqual({
  ...
  it("rejects an unknown reasoning effort", () => {
    const res = parseAgentSpec("ruby#openai/gpt-5.6-luna@max")
  ...
  it("rejects a model without a provider", () => {
    const res = parseAgentSpec("ruby#grok-4.6")
    expect(res.isOk).toBe(false)
  })
