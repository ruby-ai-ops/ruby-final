import { RubyAPI, isSupportedFileContentType } from "@ruby-ai/client"
import type {
...
const RUBY_API_BASE_URL = "https://ruby.ad"
...
/**
 * Extract the cost breakdown (in Ruby credits) of the agent message replying to
...
export interface RubyClientConfig {
  apiKey: string
  workspaceId: string
  verbose: boolean
  maxRetries: number
  ...
  if (verbose) {
    return {
      error: (args: Record<string, unknown>, message: string): void => {
        // The published @ruby-ai/client 1.2.6 build of streamAgentAnswerEvents
        ...
        if (message === "Failed parsing chunk from Ruby API") {
          return
        }
        console.error(`[RUBY ERROR] ${message}`, args)
      },
      info: (args: Record<string, unknown>, message: string): void => {
        console.error(`[RUBY INFO] ${message}`, args)
      },
      trace: (args: Record<string, unknown>, message: string): void => {
        console.error(`[RUBY TRACE] ${message}`, args)
      },
      warn: (args: Record<string, unknown>, message: string): void => {
        console.error(`[RUBY WARN] ${message}`, args)
      },
    }
  }
...
export class RubyClient {
  private client: RubyAPI
  private config: RubyClientConfig

  constructor(config: RubyClientConfig) {
    this.config = config
    this.client = new RubyAPI(
      { url: "https://ruby.ad" },
      {
  ...
  /**
   * Fetch the total cost (in Ruby credits) of the agent message replying to
  ...
  private async fetchCostCredits(
    ...
    const url = `${RUBY_API_BASE_URL}/api/v1/w/${this.config.workspaceId}/assistant/conversations/${conversationId}`
