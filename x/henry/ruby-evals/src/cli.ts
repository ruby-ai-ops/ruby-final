...
async function main(): Promise<void> {
  const program = new Command()
    .name("ruby-eval")
    .description("Evaluate Ruby agents using prompts from CSV files")
    .version("2.0.0")
  ...
  const apiKey = process.env["RUBY_API_KEY"]
  const workspaceId = process.env["RUBY_WORKSPACE_ID"]

  if (!apiKey) {
    console.error("Error: RUBY_API_KEY environment variable is not set")
    process.exit(1)
  }

  if (!workspaceId) {
    console.error("Error: RUBY_WORKSPACE_ID environment variable is not set")
    process.exit(1)
  }
  ...
  // Print configuration
  console.error(`Ruby Eval v2.0.0`)
  console.error(``)
