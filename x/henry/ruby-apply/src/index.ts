...
import { ConversationPublicType, RubyAPI } from "@ruby-ai/client";
...
async function main() {
  ...
  } else if (cmd && cmd.includes("ruby.ad")) {
    const config = await loadConfig();
    return applyCommand(cmd, config);
  }

  console.error("Usage:");
  console.error("  ruby-apply config <apiKey> <workspaceId>");
  console.error("  ruby-apply <ruby.ad assistant URL>");
  process.exit(1);
}
...
function extractConversationId(url: string): string {
  const match = url.match(/ruby\.ad\/w\/[^/]+\/assistant\/([a-zA-Z0-9]+)/);
  if (!match)
    throw new Error(
      "Invalid URL: must be a ruby.ad agent URL (format: ruby.ad/w/<workspace>/assistant/<id>)"
    );
  return match[1];
}
