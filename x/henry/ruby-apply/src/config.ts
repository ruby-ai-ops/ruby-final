...
const CONFIG_FILE = path.join(os.homedir(), ".ruby-apply.json");
...
async function loadConfig(): Promise<Config> {
  if (!(await fileSystem.exists(CONFIG_FILE))) {
    console.error(
      "Config file not found. Run: ruby-apply config <apiKey> <workspaceId>"
    );
    process.exit(1);
  }
  ...
  if (!text) {
    console.error(
      "Config file is empty. Run: ruby-apply config <apiKey> <workspaceId>"
    );
    process.exit(1);
  }
