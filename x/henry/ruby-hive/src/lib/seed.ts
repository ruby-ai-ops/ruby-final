// Fast SQL-based seeding for ruby-hive dev environments
//
...
// The SQL is defined in front/lib/dev/ruby_hive_seed.sql
...
// Keep in sync with front/scripts/seed/* (workspace created by ruby-hive seed SQL).
...
function buildDatabaseUri(envVars: Record<string, string>): string {
  return buildPostgresUri(envVars, "ruby_front");
}
...
export async function runSqlSeed(env: Environment): Promise<boolean> {
  ...
  const sqlPath = `${worktreePath}/front/lib/dev/ruby_hive_seed.sql`;
