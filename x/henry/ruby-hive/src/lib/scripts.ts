...
import { RUBY_HIVE_SCRIPTS, getServiceLogsTuiPath } from "./paths";
...
get_log_file() {
  local service="$1"
  echo "$HOME/.ruby-hive/envs/$ENV_NAME/$service.log"
}
...
restart_service() {
  ...
  ruby-hive restart "$ENV_NAME" "$service"
...
quit_service() {
  ...
  ruby-hive stop "$ENV_NAME" "$service"
  sleep 1
  switch_service
}
...
/**
 * Ensures the service logs TUI script exists at ~/.ruby-hive/scripts/service-logs-tui.sh
 * and returns its path.
 */
export async function ensureServiceLogsTui(): Promise<string> {
  await mkdir(RUBY_HIVE_SCRIPTS, { recursive: true });
