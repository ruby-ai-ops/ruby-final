...
// ruby-hive project root (where this package lives)
export const RUBY_HIVE_ROOT = resolve(dirname(import.meta.path), "../..");

// Base directories
export const RUBY_HIVE_HOME = join(homedir(), ".ruby-hive");
export const RUBY_HIVE_ENVS = join(RUBY_HIVE_HOME, "envs");
export const RUBY_HIVE_SCRIPTS = join(RUBY_HIVE_HOME, "scripts");

// Hives directory name (relative to repo root)
export const HIVES_DIR = ".hives";

// Global config
export const CONFIG_ENV_PATH = join(RUBY_HIVE_HOME, "config.env");
export const SETTINGS_PATH = join(RUBY_HIVE_HOME, "settings.json");

// Global forwarder paths (not per-env, since forwarding is global)
export const FORWARDER_PID_PATH = join(RUBY_HIVE_HOME, "forward.pid");
export const FORWARDER_LOG_PATH = join(RUBY_HIVE_HOME, "forward.log");
export const FORWARDER_STATE_PATH = join(RUBY_HIVE_HOME, "forward.json");

// Activity tracking (last-interacted environment)
export const ACTIVITY_PATH = join(RUBY_HIVE_HOME, "activity.json");

// Temporal server paths (global, not per-env)
export const TEMPORAL_PID_PATH = join(RUBY_HIVE_HOME, "temporal.pid");
export const TEMPORAL_LOG_PATH = join(RUBY_HIVE_HOME, "temporal.log");
export const TEMPORAL_DB_PATH = join(RUBY_HIVE_HOME, "temporal.db");
export const TEMPORAL_PORT = 7233;

// Shared test Postgres paths (global, not per-env)
export const TEST_POSTGRES_CONTAINER_NAME = "ruby-hive-test-postgres";
...
export const TEST_REDIS_CONTAINER_NAME = "ruby-hive-test-redis";
export const TEST_REDIS_PORT = 6479;

// Seed user configuration
export const SEED_USER_PATH = join(RUBY_HIVE_HOME, "seed-user.json");

// Per-environment paths
export function getEnvDir(name: string): string {
  return join(RUBY_HIVE_ENVS, name);
}

// Old worktree location (pre-migration), kept for backward compatibility
const OLD_WORKTREES_DIR = join(homedir(), "ruby-hive");

// Returns the worktree path for an environment.
// Explicit paths are used by externally managed worktrees adopted by ruby-hive.
// Falls back to the old ~/ruby-hive/{name} location if the new path doesn't
...
export function getDockerComposePath(): string {
  return join(RUBY_HIVE_ROOT, "docker-compose.yml");
}
...
export function getServiceLogsTuiPath(): string {
  return join(RUBY_HIVE_SCRIPTS, "service-logs-tui.sh");
}
