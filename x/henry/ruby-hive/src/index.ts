...
cli
  ...
  .option("-w, --warm", "Open with a warm tab running ruby-hive warm")
  .option(
    "-W, --wait",
...
cli
  .command("cd [name]", "Print worktree path (use with: cd $(ruby-hive cd <env>))")
  .action(async (name: string | undefined) => {
...
cli
  ...
  .example("ruby-hive forward          # Forward to current env (or prompt)")
  .example("ruby-hive forward my-env   # Forward to specific env")
  .example("ruby-hive forward status   # Show forwarding status")
  .example("ruby-hive forward stop     # Stop port forwarding")
  .action(async (name: string | undefined) => {
  ...
  .action(
    async (subcommand: string | undefined, key: string | undefined, value: string | undefined) => {
      if (!subcommand || subcommand === "list") {
        ...
        if (!key) {
          logger.error("Usage: ruby-hive env get <KEY>");
          process.exit(1);
        }
        await prepareAndRun(envGetCommand(key));
      } else if (subcommand === "set") {
        if (!key || value === undefined) {
          logger.error("Usage: ruby-hive env set <KEY> <VALUE>");
          process.exit(1);
        }
        await prepareAndRun(envSetCommand(key, value));
      } else if (subcommand === "unset") {
        if (!key) {
          logger.error("Usage: ruby-hive env unset <KEY>");
          process.exit(1);
        }
