...
export async function seedConfigCommand(postgresUri?: string): Promise<Result<void>> {
  if (!postgresUri) {
    return Err(
      new CommandError(
        "Usage: ruby-hive seed-config <postgres-uri>\n\nExample: ruby-hive seed-config 'postgres://dev:dev@localhost:5432/ruby_front'"
      )
    );
  }
  ...
  console.log();
  console.log("Next time you run 'ruby-hive warm', your database will be seeded with this user.");
  console.log();
