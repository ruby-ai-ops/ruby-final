...
// Phase 3: Start build watchers (ui and SDK)
export async function startBuildWatchers(
  ...
  try {
    // Start both ui and SDK in parallel
    await Promise.all([startService(env, "ui"), startService(env, "sdk")]);
    if (waitForReady) {
      await Promise.all([waitForServiceReady(env, "ui"), waitForServiceReady(env, "sdk")]);
    }
  } catch (error) {
