...
async function installClaudeSkills(repoRoot: string): Promise<boolean> {
  const rubyHiveClaudeDir = join(repoRoot, "x/henry/ruby-hive/.claude");
  const repoClaudeDir = join(repoRoot, ".claude");

  // Check if source .claude directory exists
  if (!(await directoryExists(rubyHiveClaudeDir))) {
    return true; // Nothing to install
  }

  // Copy skills directory (fully replacing ruby-hive skill if it exists)
  const skillsSrc = join(rubyHiveClaudeDir, "skills");
  const skillsDest = join(repoClaudeDir, "skills");
  if (await directoryExists(skillsSrc)) {
    // Remove existing ruby-hive skill to ensure clean replacement
    const rubyHiveSkillDest = join(skillsDest, "ruby-hive");
    if (await directoryExists(rubyHiveSkillDest)) {
      await rm(rubyHiveSkillDest, { recursive: true });
    }
...
// Install and link ruby-hive if needed
async function updateRubyHive(rubyHivePath: string, needsInstall: boolean): Promise<Result<void>> {
  if (!needsInstall) {
    logger.info("ruby-hive dependencies up to date (no changes detected)");
    return Ok(undefined);
  }

  logger.step("Installing x/henry/ruby-hive dependencies...");
  const installSuccess = await runBunInstall(rubyHivePath);
  if (!installSuccess) {
    return Err(new CommandError("Failed to run bun install for ruby-hive"));
  }
  logger.success("x/henry/ruby-hive dependencies installed");

  logger.step("Linking x/henry/ruby-hive...");
  const linkSuccess = await runBunLink(rubyHivePath);
  if (!linkSuccess) {
    return Err(new CommandError("Failed to run bun link for ruby-hive"));
  }
  logger.success("ruby-hive linked globally");
  return Ok(undefined);
}
...
export async function syncCommand(options: SyncOptions = {}): Promise<Result<void>> {
  ...
  if (!repoRoot) {
    return Err(new CommandError("Not in a git repository. Run from within the Ruby repo."));
  }
  ...
  // Install and link ruby-hive if needed
  const rubyHiveResult = await updateRubyHive(rubyHiveDir, needsBunInstall);
  if (!rubyHiveResult.ok) {
    return rubyHiveResult;
  }
