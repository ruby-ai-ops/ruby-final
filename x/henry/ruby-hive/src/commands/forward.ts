...
export async function forwardStatusCommand(): Promise<Result<void>> {
  ...
  if (status.running && status.state) {
    ...
    console.log();
    console.log("Run 'ruby-hive forward NAME' to start forwarding to an environment.");
  }
...
async function forwardToEnv(name: string): Promise<Result<void>> {
  ...
  if (!proxyRunning) {
    return Err(
      new CommandError(
        `Environment '${name}' does not have the proxy running. Run 'ruby-hive warm ${name}' first.`
      )
    );
  }
