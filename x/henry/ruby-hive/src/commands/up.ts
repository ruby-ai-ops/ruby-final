...
async function startTemporalIfNeeded(): Promise<Result<void>> {
  ...
  if (temporalStatus.running) {
    ...
    return Err(
      new CommandError(
        `Temporal is already running externally on port ${TEMPORAL_PORT}. Stop it first to use ruby-hive managed temporal.`
      )
    );
  }
...
export async function upCommand(options: UpOptions = {}): Promise<Result<void>> {
  ...
  logger.info("Starting ruby-hive managed services...");
  console.log();

  // Check preconditions
  const preconditions = await checkMainRepoPreconditions(repoRoot, { commandName: "ruby-hive up" });
  if (!preconditions.ok) {
  ...
  if (options.attach) {
    ...
    console.log("  ruby-hive up -a             # Attach to main session");
    console.log("  ruby-hive spawn <name>      # Create a new environment");
    console.log("  ruby-hive list              # List environments");
    console.log();
  }
