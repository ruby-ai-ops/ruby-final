...
export async function flagCommand(
  ...
  if (stateInfo.state !== "warm") {
    ...
    return Err(
      new CommandError(
        `Environment '${env.name}' is not warm (current state: ${stateInfo.state}). Run 'ruby-hive warm ${env.name}' first.`
      )
    );
  }
  ...
  if (unknownFlags.length > 0) {
    return Err(
      new CommandError(
        `Unknown feature flag(s): ${unknownFlags.join(", ")}. Run 'ruby-hive flag ${env.name}' with no flag name to pick from the list.`
      )
    );
  }
