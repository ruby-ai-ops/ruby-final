...
async function checkRepo(): Promise<CheckResult> {
  ...
  return {
    ...
    fix: "Run ruby-hive from within the Ruby repository",
  };
}
...
export async function setupCommand(options: SetupOptions = {}): Promise<Result<void>> {
  ...
  if (allOk) {
    ...
    console.log("  ruby-hive up            # Start temporal server + sync main repo");
  ...
  if (options.nonInteractive) {
    logger.warn("Some prerequisites are missing. Please install them to use ruby-hive.");
  } else {
    logger.info("Once prerequisites are met, run: ruby-hive up");
  }
