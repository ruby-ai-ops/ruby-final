...
describe("docker", () => {
  describe("generateDockerComposeOverride", () => {
    ...
    it("generates correct volume names", () => {
      ...
      expect(override.services.db.volumes).toContain(
        "ruby-hive-my-feature-pgsql:/var/lib/postgresql/data"
      );
      expect(override.services.qdrant.volumes).toContain(
        "ruby-hive-my-feature-qdrant:/qdrant/storage"
      );
      expect(override.services.elasticsearch.volumes).toContain(
        "ruby-hive-my-feature-elasticsearch:/usr/share/elasticsearch/data"
      );
    });
    ...
    it("declares all volumes at top level", () => {
      ...
      expect(override.volumes).toHaveProperty("ruby-hive-env-a-pgsql");
      expect(override.volumes).toHaveProperty("ruby-hive-env-a-qdrant");
      expect(override.volumes).toHaveProperty("ruby-hive-env-a-elasticsearch");
    });
  });

  describe("getDockerProjectName", () => {
    it("returns correct project name", () => {
      expect(getDockerProjectName("test")).toBe("ruby-hive-test");
      expect(getDockerProjectName("my-feature")).toBe("ruby-hive-my-feature");
    });
  });
  ...
  describe("getVolumeNames", () => {
    it("returns all volume names", () => {
      ...
      expect(volumes).toContain("ruby-hive-test-pgsql");
      expect(volumes).toContain("ruby-hive-test-qdrant");
      expect(volumes).toContain("ruby-hive-test-elasticsearch");
    });
  });
});
