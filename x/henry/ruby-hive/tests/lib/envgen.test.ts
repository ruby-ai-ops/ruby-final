...
describe("envgen", () => {
  describe("generateEnvSh", () => {
    ...
    it("includes shebang and header comment", () => {
      ...
      expect(content).toContain("Generated by ruby-hive for environment: test");
    });
    ...
    it("sources global config.env", () => {
      ...
      expect(content).toContain(".ruby-hive/config.env");
    });
    ...
    it("exports temporal namespaces", () => {
      const content = generateEnvSh("my-feature", ports);
      expect(content).toContain("export TEMPORAL_NAMESPACE=ruby-hive-my-feature");
      expect(content).toContain("export TEMPORAL_AGENT_NAMESPACE=ruby-hive-my-feature-agent");
      expect(content).toContain(
        "export TEMPORAL_CONNECTORS_NAMESPACE=ruby-hive-my-feature-connectors"
      );
      expect(content).toContain(
        "export TEMPORAL_RELOCATION_NAMESPACE=ruby-hive-my-feature-relocation"
      );
    });
    ...
    it("exports inter-service URLs", () => {
      ...
      expect(content).toContain("export RUBY_FRONT_API=http://localhost:10000");
      expect(content).toContain("export RUBY_CLIENT_FACING_URL=http://localhost:10000");
      expect(content).toContain("export NEXT_PUBLIC_RUBY_API_URL=http://localhost:10000");
      expect(content).toContain(
        "export NEXT_PUBLIC_RUBY_STATIC_WEBSITE_URL=http://localhost:10000"
      );
      expect(content).toContain("export RUBY_AUTH_REDIRECT_BASE_URL=http://localhost:3000");
    ...
    it("exports database URIs with correct port", () => {
      const content = generateEnvSh("test", ports);
      expect(content).toContain(
        "export FRONT_DATABASE_URI=postgres://dev:dev@localhost:10432/ruby_front"
      );
      expect(content).toContain(
        "export CORE_DATABASE_URI=postgres://dev:dev@localhost:10432/ruby_api"
      );
      expect(content).toContain(
        "export CONNECTORS_DATABASE_URI=postgres://dev:dev@localhost:10432/ruby_connectors"
      );
      expect(content).toContain(
        "export OAUTH_DATABASE_URI=postgres://dev:dev@localhost:10432/ruby_oauth"
      );
    });
    ...
    it("uses different ports for second environment", () => {
      ...
      expect(content).toContain("FRONT_DATABASE_URI=postgres://dev:dev@localhost:11432/ruby_front");
    });
    ...
    it("uses environment name in temporal namespaces", () => {
      ...
      expect(content1).toContain("TEMPORAL_NAMESPACE=ruby-hive-env-a");
      expect(content2).toContain("TEMPORAL_NAMESPACE=ruby-hive-env-b");
    });
    ...
    it("uses front port for RUBY_AUTH_REDIRECT_BASE_URL when dynamicWorkosRedirect is enabled", () => {
      const content = generateEnvSh("test", ports, { dynamicWorkosRedirect: true });
      expect(content).toContain("export RUBY_AUTH_REDIRECT_BASE_URL=http://localhost:10000");
      expect(content).not.toContain("export RUBY_AUTH_REDIRECT_BASE_URL=http://localhost:3000");
    });

    it("defaults RUBY_AUTH_REDIRECT_BASE_URL to :3000 when dynamicWorkosRedirect is false", () => {
