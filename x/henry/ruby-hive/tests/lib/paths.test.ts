...
describe("paths", () => {
  ...
  describe("base directory constants", () => {
    it("sets RUBY_HIVE_HOME to ~/.ruby-hive", () => {
      expect(RUBY_HIVE_HOME).toBe(join(home, ".ruby-hive"));
    });

    it("sets RUBY_HIVE_ENVS to ~/.ruby-hive/envs", () => {
      expect(RUBY_HIVE_ENVS).toBe(join(home, ".ruby-hive", "envs"));
    });
    ...
    it("sets CONFIG_ENV_PATH to ~/.ruby-hive/config.env", () => {
      expect(CONFIG_ENV_PATH).toBe(join(home, ".ruby-hive", "config.env"));
    });
  });

  describe("getEnvDir", () => {
    it("returns correct path for environment", () => {
      expect(getEnvDir("test")).toBe(join(home, ".ruby-hive", "envs", "test"));
    });

    it("handles environment names with hyphens", () => {
      expect(getEnvDir("my-feature")).toBe(join(home, ".ruby-hive", "envs", "my-feature"));
    });
  });
  ...
  describe("getEnvFilePath", () => {
    it("returns path to env.sh in environment directory", () => {
      expect(getEnvFilePath("test")).toBe(join(home, ".ruby-hive", "envs", "test", "env.sh"));
    });
  });
  ...
  describe("getDockerOverridePath", () => {
    it("returns path to docker-compose.override.yml", () => {
      expect(getDockerOverridePath("test")).toBe(
        join(home, ".ruby-hive", "envs", "test", "docker-compose.override.yml")
      );
    });
  });
  ...
  describe("getMetadataPath", () => {
    it("returns path to metadata.json", () => {
      expect(getMetadataPath("test")).toBe(
        join(home, ".ruby-hive", "envs", "test", "metadata.json")
      );
    });
  });
  ...
  describe("getPortsPath", () => {
    it("returns path to ports.json", () => {
      expect(getPortsPath("test")).toBe(join(home, ".ruby-hive", "envs", "test", "ports.json"));
    });
  });
  ...
  describe("getInitializedMarkerPath", () => {
    it("returns path to initialized marker file", () => {
      expect(getInitializedMarkerPath("test")).toBe(
        join(home, ".ruby-hive", "envs", "test", "initialized")
      );
    });
  });
  ...
  describe("getPidPath", () => {
    it("returns correct PID file path for service", () => {
      expect(getPidPath("test", "front")).toBe(
        join(home, ".ruby-hive", "envs", "test", "front.pid")
      );
      expect(getPidPath("test", "core")).toBe(join(home, ".ruby-hive", "envs", "test", "core.pid"));
      expect(getPidPath("test", "sdk")).toBe(join(home, ".ruby-hive", "envs", "test", "sdk.pid"));
    });

    it("handles service names with hyphens", () => {
      expect(getPidPath("test", "front-workers")).toBe(
        join(home, ".ruby-hive", "envs", "test", "front-workers.pid")
      );
    });
  });
  ...
  describe("getLogPath", () => {
    it("returns correct log file path for service", () => {
      expect(getLogPath("test", "front")).toBe(
        join(home, ".ruby-hive", "envs", "test", "front.log")
      );
      expect(getLogPath("test", "core")).toBe(join(home, ".ruby-hive", "envs", "test", "core.log"));
    });

    it("handles service names with hyphens", () => {
      expect(getLogPath("test", "front-workers")).toBe(
        join(home, ".ruby-hive", "envs", "test", "front-workers.log")
      );
    });
  });
  ...
  describe("findRepoRoot", () => {
    const testDir = "/tmp/ruby-hive-test-findRepoRoot";
