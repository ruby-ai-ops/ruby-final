...
describe("registry", () => {
  ...
  describe("WARM_SERVICES", () => {
    it("excludes ui, sdk, viz, storybook and sqlite-worker from warm services", () => {
      expect(WARM_SERVICES).not.toContain("ui");
    ...
    it("includes all other services", () => {
      ...
      expect(WARM_SERVICES).toContain("front-spa-admin");
      expect(WARM_SERVICES).toContain("front-spa-app");
    });

    it("has 9 services (all except ui, sdk, viz, storybook, sqlite-worker)", () => {
