...
describe("formatForwarderMapping", () => {
  ...
  it("deep-links SPA rows into the seeded workspace", () => {
    ...
    const admin = stripHyperlinks(formatForwarderMapping(mappingNamed("front-spa-admin"), BASE_PORT));
    expect(admin).toContain(`http://localhost:3010/${WORKSPACE_ID}`);
    expect(admin).toContain(`http://localhost:12010/${WORKSPACE_ID}`);
  });
