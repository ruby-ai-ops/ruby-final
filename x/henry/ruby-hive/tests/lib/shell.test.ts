...
describe("shell", () => {
  describe("buildShell", () => {
    ...
    it("handles multiple commands with env and nvm", () => {
      const result = buildShell({
        sourceEnv: "/home/user/.ruby-hive/envs/test/env.sh",
        sourceNvm: true,
        run: ["npm install", "npm run build", "npm run dev"],
      });

      expect(result).toBe(
        'set -e && set -o pipefail && source \'/home/user/.ruby-hive/envs/test/env.sh\' && export NVM_DIR="${NVM_DIR:-$HOME/.nvm}" && [ -s "$NVM_DIR/nvm.sh" ] && . "$NVM_DIR/nvm.sh" && nvm use && npm install && npm run build && npm run dev'
      );
    });
    ...
    it("handles complex commands with special characters", () => {
      const result = buildShell({
        run: "TEMPORAL_NAMESPACE=ruby-hive-test npx tsx src/start.ts -p 10002",
      });
      expect(result).toBe(
        "set -e && set -o pipefail && TEMPORAL_NAMESPACE=ruby-hive-test npx tsx src/start.ts -p 10002"
      );
    });
    ...
    it("builds correct command for front service", () => {
      const result = buildShell({
        sourceEnv: "/home/user/.ruby-hive/envs/my-env/env.sh",
        sourceNvm: true,
        run: "npm run dev",
      });

      expect(result).toContain("set -e && set -o pipefail");
      expect(result).toContain("source '/home/user/.ruby-hive/envs/my-env/env.sh'");
    ...
    it("builds correct command for core service (no nvm)", () => {
      ...
      expect(result).toBe(
        "set -e && set -o pipefail && source '/home/user/.ruby-hive/envs/my-env/env.sh' && cargo run --bin core-api"
      );
    });
  });
});
