...
**IMPORTANT**: Always use `bun run` commands from within the `x/henry/ruby-hive` directory. Do NOT run `tsc`, `biome`, or other tools directly - use the npm scripts which ensure correct configuration.
...
5. **Managed Temporal** - `ruby-hive up` runs Temporal as a daemon, namespaces created per env
6. **Multiplexer support** - Both zellij (default) and tmux are supported via `~/.ruby-hive/settings.json`

## Commands Reference

> **Tip**: Run `ruby-hive <command> --help` for all available options (e.g., `--force`, `--compact`, `--unified-logs`).
...
ruby-hive uses the main Ruby repo as a cache source for:
...
For `front` and `connectors`, ruby-hive creates a **shallow copy** of node_modules:
- A real directory with symlinks to all packages from the main repo
- `@ruby-ai/client` is overridden to point to the worktree's SDK

This ensures correct SDK type resolution when your branch has newer types than main.

**Note**: To run `npm install` in a ruby-hive worktree, you must first delete `node_modules`:
```bash
...
Check cache status:
```bash
ruby-hive cache            # Show what's cached
ruby-hive sync             # Pull main, rebuild binaries, refresh deps
```
