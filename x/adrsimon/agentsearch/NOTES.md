...
npx tsx scripts/eval.ts --queries assets/eval_queries_ruby.json --profile assets/profile_<id>.json \
...
- `description.subsequence`. Dead weight in the mapping since the clause was dropped. It matched almost anything (`*i*n*v*o*i*c*e*` hits "Incident investigation assistant for Ruby using Datadog logs"). Remove it.
