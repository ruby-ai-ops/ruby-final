...
A native Swift/SwiftUI iOS app for Ruby with WorkOS authentication.
...
The Xcode project (`Ruby.xcodeproj`) is generated by XcodeGen from `project.yml` and is
...
| `prod`    | `Prod`       | debug      | `https://ruby.ad`       |
| `release` | `Release`    | release    | `https://ruby.ad`       |
...
All three flavors share the bundle id `com.ruby.mobile`, so they can't be installed
...
make generate             # (re)generate Ruby.xcodeproj from project.yml
```
...
The app is signed with automatic signing against the Ruby Apple Developer team
(`DEVELOPMENT_TEAM` is set in `project.yml`). The bundle id `com.ruby.mobile` and the
...
```bash
make generate && open Ruby.xcodeproj
