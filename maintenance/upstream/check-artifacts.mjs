...
const roots = ['sdks/js/dist','ui/dist','front/dist','front-api/dist','front-spa/dist','marketing/.next-build','marketing/public/static/workspace-demo','connectors/dist','extension/build','viz/.next','cli/ruby-cli/dist'];
const findings=[];
function scan(root) {
  ...
  for (const entry of fs.readdirSync(root,{withFileTypes:true})) {
    ...
    if (/@dust(?:-tt)?\/|dust\.tt|dust-tt|DustAPI|DustHive|DUST_|\bdsbx\b|@ruby-ai\/sparkle/.test(text)) findings.push(name);
  }
}
roots.forEach(scan);
