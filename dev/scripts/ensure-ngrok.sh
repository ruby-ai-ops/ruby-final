...
NGROK_CONFIG_FILE="${NGROK_CONFIG_FILE:-${RUBY_INFRA_LOG_DIR}/ngrok-ruby-dev.yml}"
NGROK_DEV_ID_FILE="${NGROK_DEV_ID_FILE:-${RUBY_INFRA_LOG_DIR}/ngrok-dev-id}"
...
front_endpoint_url() {
  printf 'https://ruby-dev-front-%s.%s' "$1" "${NGROK_DOMAIN_SUFFIX}"
}

viz_endpoint_url() {
  printf 'https://ruby-dev-viz-%s.%s' "$1" "${NGROK_DOMAIN_SUFFIX}"
}
