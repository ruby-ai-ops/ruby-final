...
const setupRevisionedFile = async () => {
  ...
  const revision = loaded.headers.get(RUBY_FILE_REVISION_HEADER);
  assert(revision);
...
describe("conditional updates through Files paths", () => {
  ...
  it("reads ordinary JSON, conditionally saves it, and reopens the saved bytes", async () => {
    ...
    const saved = await request(workspace, path, {
      method: "PUT",
      headers: {
        [RUBY_IF_REVISION_MATCH_HEADER]: revision,
    ...
    const savedRevision = saved.headers.get(RUBY_FILE_REVISION_HEADER);
    expect(savedRevision).not.toBe(revision);

    const reopened = await request(workspace, path);
    expect(await reopened.text()).toBe(edited);
    expect(reopened.headers.get(RUBY_FILE_REVISION_HEADER)).toBe(savedRevision);
  });
    ...
    const results = await Promise.all(
      edits.map((body) =>
        request(workspace, path, {
          method: "PUT",
          headers: { [RUBY_IF_REVISION_MATCH_HEADER]: revision },
          body,
        })
      )
    );
  ...
  it("pins streamed bytes to the returned revision when another writer updates during the read", async () => {
    ...
    expect(response.headers.get(RUBY_FILE_REVISION_HEADER)).toBe(revision);
  });
  ...
  it("returns this write's revision even if another writer saves before the response", async () => {
    ...
    expect(saved.headers.get(RUBY_FILE_REVISION_HEADER)).not.toBe(
      reloaded.headers.get(RUBY_FILE_REVISION_HEADER)
    );
  });
  ...
  it("does not expose private Pod files to another workspace member", async () => {
    ...
    const saved = await request(workspace, path, {
      method: "PUT",
      headers: { [RUBY_IF_REVISION_MATCH_HEADER]: "1" },
      body: '{"unauthorized":true}',
    });
    expect(saved.status).toBe(403);
    expect(fileStorageMock.getObject(mountPath)).toBe("{}");
  ...
  it("rejects a conditional write when storage cannot enforce the revision", async () => {
    ...
    const saved = await request(workspace, path, {
      method: "PUT",
      headers: {
        [RUBY_IF_REVISION_MATCH_HEADER]: "1",
        "Content-Type": "application/json",
      },
      body: "{}",
    });
  ...
  it("preserves overwrite behavior for unconditional writes", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get(RUBY_FILE_REVISION_HEADER)).toBeTruthy();
