...
// Mounted at /api/admin/connectors/:connectorId/redirect. adminAuth is applied
// by the parent admin sub-app.
const app = adminApp();
...
app.get(
  ...
  async (ctx): HandlerResult<GetConnectorRedirectResponse> => {
    ...
    return ctx.json({
      redirectUrl: `/admin/${connector.workspaceId}/data_sources/${connector.dataSourceId}`,
    });
  }
);
