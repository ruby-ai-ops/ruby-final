...
// Mounted at /api/admin/plugins/:pluginId/run.
//
// We extend the admin context with `HttpBindings` so we can hand the raw Node
// `IncomingMessage` (exposed by `@hono/node-server` on `ctx.env.incoming`) to
// `formidable.parse(...)` — matching the Next handler.
const app = createHono<AdminCtx & { Bindings: HttpBindings }>();
...
app.post(
  ...
  async (ctx): HandlerResult<AdminRunPluginResponseBody> => {
    ...
    if (plugin.manifest.requiredRoles) {
      ...
      if (!hasAdminRole(userRoles, plugin.manifest.requiredRoles)) {
        return apiError(ctx, {
          status_code: 403,
          api_error: {
