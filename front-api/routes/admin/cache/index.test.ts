...
const lookupUrl = `/api/admin/cache?resourceId=group_permissions_by_workspace&params=${encodeURIComponent(
  JSON.stringify({ workspaceModelId: String(WORKSPACE_MODEL_ID) })
)}`;

describe("Admin cache: group permissions", () => {
  beforeEach(async () => {
...
describe("DELETE /api/admin/cache", () => {
  ...
  it("deletes the new and previous keys", async () => {
    await createAdminApiMockRequest({
      method: "DELETE",
      isSuperUser: true,
    });
    ...
    const response = await honoApp.request(`/api/admin/cache?${query}`, {
      method: "DELETE",
    });
