...
import type {
  ...
  DeleteAdminCacheResponseBody,
  GetAdminCacheResponseBody,
  RedisCacheResult,
} from "@app/types/api/admin/cache";
import { isString } from "@app/types/shared/utils/general";
import { safeParseJSON } from "@app/types/shared/utils/json_utils";
import { getAdminCacheOperations } from "@front-api/lib/api/admin/cache_catalog";
...
// Mounted at /api/admin/cache. adminAuth is applied by the parent admin sub-app.
const app = adminApp();
...
function resolveCacheKey(
  ...
  if (isString(resourceId)) {
    const operations = getAdminCacheOperations(resourceId);
    if (!operations) {
      return {
        err: apiError(ctx, {
          status_code: 400,
...
/** @ignoreswagger */
app.get("/", async (ctx): HandlerResult<GetAdminCacheResponseBody> => {
  ...
  const lookupKey = async (
    ...
    return runFn({ origin: "admin_cache_lookup" }, async (client) => {
...
app.delete("/", async (ctx): HandlerResult<DeleteAdminCacheResponseBody> => {
  const r = resolveCacheKey(ctx);
  if ("err" in r) {
    return r.err;
  }
...
app.delete(
  "/all",
  async (ctx): HandlerResult<DeleteAllAdminCacheResponseBody> => {
    const resourceId = ctx.req.query("resourceId");
    if (!isString(resourceId)) {
      return apiError(ctx, {
        status_code: 400,
    ...
    const deletedCount = await runOnRedisCache(
      { origin: "admin_cache_invalidation" },
      async (client) => {
        let count = 0;
        let batch: string[] = [];
    ...
    logger.info(
      ...
      "Admin cache bulk invalidation performed"
    );
