...
function fileUrl(workspaceId: string, sId: string) {
  return `/api/admin/workspaces/${workspaceId}/files/${sId}`;
}

describe("GET /api/admin/workspaces/:wId/files/:sId", () => {
  it("returns a Frames v2 file instead of rejecting it", async () => {
    ...
    // A Frame v2's own bytes are its manifest JSON, which Admin's dedicated Frame view never
