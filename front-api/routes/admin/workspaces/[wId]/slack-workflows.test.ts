...
async function setupTest({
  ...
  const setup = await createAdminApiMockRequest({
    isSuperUser,
    role: "admin",
  });
...
function adminSlackWorkflowsRequest(
  wId: string,
  { method, body }: { method: string; body?: Record<string, unknown> }
) {
  return honoApp.request(`/api/admin/workspaces/${wId}/slack-workflows`, {
    method,
...
describe("GET /api/admin/workspaces/[wId]/slack-workflows", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("returns 401 for non super users", async () => {
    ...
    const response = await adminSlackWorkflowsRequest(workspace.sId, {
      method: "GET",
    });
...
describe("DELETE /api/admin/workspaces/[wId]/slack-workflows", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
