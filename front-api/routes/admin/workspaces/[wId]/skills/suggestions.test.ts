...
describe("POST /api/admin/workspaces/:wId/skills/suggestions", () => {
  it("stores inline tool and skill references", async () => {
    const { auth, workspace, globalSpace } = await createAdminApiMockRequest({
      method: "POST",
      isSuperUser: true,
      role: "admin",
    });
    ...
    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/skills/suggestions`,
      {
        method: "POST",
