...
import { listDatabasesOnReadySandbox } from "@app/lib/api/sandbox_functions/rbx_db";
...
// Listing databases wakes the Frame sandbox and runs `rbx db list` inside it.
vi.mock(import("@app/lib/api/sandbox_functions/rbx_db"), async (orig) => {
...
function databasesUrl(workspaceId: string, frameId: string) {
  return `/api/admin/workspaces/${workspaceId}/frames/${frameId}/databases`;
}

describe("GET /api/admin/workspaces/:wId/frames/:frameId/databases", () => {
  ...
  it("returns 500 when `rbx db list` fails", async () => {
    const { workspace, frame } = await makeTestFrameFunction({
      isSuperUser: true,
    });
