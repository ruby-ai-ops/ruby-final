...
// Mounted at
// /api/admin/workspaces/:wId/frames/:frameId/functions/:functionId/invocations.
...
app.get(
  ...
  async (ctx): HandlerResult<AdminListSandboxFunctionInvocations> => {
...
app.get(
  ...
  async (ctx): HandlerResult<AdminGetSandboxFunctionInvocation> => {
    const auth = ctx.get("auth");
    const frameFunction = ctx.get("frameFunction");
...
app.get(
  ...
  async (ctx): HandlerResult<AdminGetSandboxFunctionMCPActionOutput> => {
    const auth = ctx.get("auth");
    const frameFunction = ctx.get("frameFunction");
