...
function functionsUrl(workspaceId: string, frameId: string) {
  return `/api/admin/workspaces/${workspaceId}/frames/${frameId}/functions`;
}

describe("GET /api/admin/workspaces/:wId/frames/:frameId/functions", () => {
  ...
  it("404s for a Frame in another workspace", async () => {
    ...
    const { workspace: otherWorkspace } = await createAdminApiMockRequest({
      isSuperUser: true,
      role: "admin",
    });
