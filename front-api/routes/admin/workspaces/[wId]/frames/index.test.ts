...
function framesUrl(workspaceId: string) {
  return `/api/admin/workspaces/${workspaceId}/frames`;
}

describe("GET /api/admin/workspaces/:wId/frames", () => {
  ...
  it("does not leak frames from another workspace", async () => {
    // Order matters: each makeTestFrameFunction call re-mocks admin CF Access, so the target
