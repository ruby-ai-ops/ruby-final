...
): AdminSearchWorkspaceMembers["members"] {
...
// Mounted at /api/admin/workspaces/:wId/memberships.
const app = adminApp();

/** @ignoreswagger */
// Admin-only member search for plugin comboboxes (name/email prefix via Elasticsearch).
app.get("/search", validate("query", SearchMembersQuerySchema), async (ctx) => {
  ...
  if (searchResult.isOk()) {
    ...
    } satisfies AdminSearchWorkspaceMembers);
  }
...
/** @ignoreswagger */
app.get("/", async (ctx): HandlerResult<AdminGetMemberships> => {
  const auth = ctx.get("auth");
