...
async function setupMessage() {
  const { auth, user, workspace } = await createAdminApiMockRequest({
    isSuperUser: true,
    role: "user",
  });
...
function messageConsumptionUrl({
  ...
  return `/api/admin/workspaces/${workspaceId}/conversations/${conversationId}/messages/${messageId}/consumption`;
}

describe("GET /api/admin/workspaces/:wId/conversations/:cId/messages/:mId/consumption", () => {
  it("returns the app message attribution plus model details to a Admin superuser", async () => {
