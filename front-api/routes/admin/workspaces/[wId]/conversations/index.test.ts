...
describe("GET /api/admin/workspaces/:wId/conversations", () => {
  it("pages the agent conversations by offset, newest first", async () => {
    const { auth, workspace } = await createAdminApiMockRequest({
      isSuperUser: true,
      role: "admin",
    });
    ...
    const url = `/api/admin/workspaces/${workspace.sId}/conversations?agentId=${agent.sId}`;
  ...
  it("returns an empty page past the end", async () => {
    ...
    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/conversations?agentId=${agent.sId}&limit=25&offset=100`
    );
  ...
  it("rejects an unknown order column", async () => {
    ...
    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/conversations?agentId=${agent.sId}&orderColumn=hasError`
    );
  ...
  it("pages within the created-at window", async () => {
    ...
    const url = `/api/admin/workspaces/${workspace.sId}/conversations?agentId=${agent.sId}&from=2026-01-01&to=2026-01-31&limit=1`;
  ...
  it("rejects a malformed date bound", async () => {
    ...
    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/conversations?agentId=${agent.sId}&from=not-a-date`
    );
  ...
  it("defaults the paging and ordering when none is provided", async () => {
    ...
    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/conversations?agentId=${agent.sId}`
    );
  ...
  it("rejects a request with no agent, trigger or reinforced skill", async () => {
    const { workspace } = await createAdminApiMockRequest({
      isSuperUser: true,
      role: "admin",
    });

    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/conversations`
    );
