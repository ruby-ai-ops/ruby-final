...
async function createGroupWithMember() {
  const { workspace, auth, user } = await createAdminApiMockRequest({
    isSuperUser: true,
    role: "admin",
  });
  const group = await GroupFactory.regularManual(
    workspace,
    "Admin member count group"
  );
...
describe("GET /api/admin/workspaces/:wId/groups/:groupId/details", () => {
  it("excludes revoked workspace members from the group count", async () => {
    const { workspace, group } = await createGroupWithMember();

    const response = await honoApp.request(
      `/api/admin/workspaces/${workspace.sId}/groups/${group.sId}/details`
    );
