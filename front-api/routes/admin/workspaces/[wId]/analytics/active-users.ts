...
export type AdminGetWorkspaceActiveUsersResponse = {
  points: ActiveUsersMetricsPoint[];
};

// Mounted at /api/admin/workspaces/:wId/analytics/active-users.
const app = adminApp();
...
app.get("/", validate("query", QuerySchema), async (ctx) => {
  ...
  const body: AdminGetWorkspaceActiveUsersResponse = { points: result.value };
