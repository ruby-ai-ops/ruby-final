...
} from "@app/lib/api/admin/plugins/workspaces/manage_programmatic_usage_configuration";
...
export interface AdminUpgradeEnterpriseSuccessResponseBody {
  success: boolean;
}

// Mounted at /api/admin/workspaces/:wId/upgrade_enterprise.
const app = adminApp();

/** @ignoreswagger */
app.post(
  "/",
  async (ctx): HandlerResult<AdminUpgradeEnterpriseSuccessResponseBody> => {
    ...
    try {
      await SubscriptionResource.adminUpgradeWorkspaceToEnterprise(
        auth,
        validated
      );
