...
import type { GetAdminWorkspacesResponseBody } from "@app/types/api/admin/workspaces";
...
// Note: the parent admin/index.ts already applies adminAuth (super-user gate).
...
/** @ignoreswagger */
app.get("/", async (ctx): HandlerResult<GetAdminWorkspacesResponseBody> => {
  ...
  let planTypeFilter: AdminPlanTypeFilter | undefined;
  if (planTypeQuery !== undefined) {
    if (!isAdminPlanTypeFilter(planTypeQuery)) {
      return apiError(ctx, {
        status_code: 400,
        api_error: {
          type: "invalid_request_error",
          message: `The request query is invalid, expects { planType: one of ${ADMIN_PLAN_TYPE_FILTERS.join(", ")} }.`,
        },
      });
    }
          ...
          include: [
            {
              model: PlanModel,
              as: "plan",
              where: buildAdminPlanCodeWhere(planTypeFilter),
              required: true,
            },
          ],
        }
