...
function searchWorkspaces(search: string, limit: string) {
  ...
  return honoApp.request(`/api/admin/workspaces?${params.toString()}`);
}

function fetchWorkspaces(params: Record<string, string>) {
  return honoApp.request(
    `/api/admin/workspaces?${new URLSearchParams(params).toString()}`
  );
}

describe("GET /api/admin/workspaces — workspace name search", () => {
  it("matches by prefix", async () => {
    const workspace = await createNamedWorkspace("Zorbix Industries");
    await createAdminApiMockRequest({ isSuperUser: true, workspace });
...
describe("GET /api/admin/workspaces — plan type filter", () => {
...
describe("GET /api/admin/workspaces — pagination", () => {
  it("pages through results via offset, and reports hasMore correctly", async () => {
    ...
    await createAdminApiMockRequest({ isSuperUser: true, workspace: alpha });
