import { canCreateCoupon, createCoupon } from "@app/lib/api/admin/coupons";
...
export type GetAdminCouponsResponseBody = {
  coupons: AdminCouponWireShape[];
};

export type CreateAdminCouponResponseBody = {
  coupon: AdminCouponWireShape;
};

// Mounted at /api/admin/coupons. adminAuth is applied by the parent admin
// sub-app.
const app = adminApp();

/** @ignoreswagger */
app.get("/", async (ctx): HandlerResult<GetAdminCouponsResponseBody> => {
...
app.post(
  ...
  async (ctx): HandlerResult<CreateAdminCouponResponseBody> => {
      ...
      return apiError(ctx, {
        ...
        api_error: {
          ...
          message:
            ...
            "Switch the Admin region to US and retry.",
        },
      });
    }
