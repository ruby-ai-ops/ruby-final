import { listDegradableEndpoints } from "@app/lib/api/admin/degraded_models";
...
function postEndpoints(endpoints: unknown[]) {
  return honoApp.request("/api/admin/degraded_models", {
    method: "POST",
...
async function getDegradedEndpoints() {
  const response = await honoApp.request("/api/admin/degraded_models");
...
describe("GET /api/admin/degraded_models", () => {
  ...
  it("returns 401 when the user is not a super user", async () => {
    await createAdminApiMockRequest({ isSuperUser: false });
...
describe("POST /api/admin/degraded_models", () => {
