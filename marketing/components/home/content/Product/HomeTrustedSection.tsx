...
const CASE_STUDIES: Record<string, string> = {
  ...
  pennylane: "/customers/pennylane-ruby-customer-support-journey",
  ...
  spendesk:
    "/customers/how-spendesk-achieved-90-ai-adoption-in-6-months-with-ruby",
  wakam:
    ...
    <section className="flex w-full items-center justify-center bg-linear-to-b from-background via-blue-50/40 to-blue-100/60 pb-20 pt-12">
      ...
      <div className="mx-auto flex w-full max-w-[1280px] flex-col items-center justify-center gap-12 text-center">
        ...
        <HomeReveal delay={120} className="w-full">
          <MarqueeMaskWrap>
            <div className="home-trusted-track flex w-max items-end gap-x-10 sm:gap-x-20 lg:gap-x-24">
              {marqueeLogos.map((logo, idx) => {
                const caseStudyUrl =
                  CASE_STUDIES[logo.name.toLowerCase().replace(/\s+/g, "")];
                const itemClassName =
                  "home-trusted-item flex min-h-11 flex-shrink-0 flex-col items-center gap-1";
                const inner = (
                  <>
