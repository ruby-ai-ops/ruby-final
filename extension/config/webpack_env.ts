...
// `import.meta.env.VITE_RUBY_API_URL*`. Vite injects those in the SPA build, but
...
// extension's RUBY_API_URL_US / RUBY_API_URL_EU config (loaded from
...
export function getImportMetaEnv(envPath: string): Record<string, string> {
  ...
  const cell00002Url = resolve("RUBY_API_URL_CELL_00002");

  return {
    VITE_RUBY_API_URL: usUrl,
    VITE_RUBY_API_URL_US: usUrl,
    VITE_RUBY_API_URL_EU: euUrl,
    VITE_RUBY_API_URL_CELL_00002: cell00002Url,
    VITE_RUBY_REGION: "us-central1",
  };
}
