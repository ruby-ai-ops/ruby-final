#!/usr/bin/env bash
# Guard: mprocs must be launched via a start wrapper (prep + RUBY_USE_START_MPROCS).
set -euo pipefail

if [ -z "${RUBY_USE_START_MPROCS:-}" ]; then
  echo ""
  ...
  echo "       Wrappers set up deps and export RUBY_USE_START_MPROCS."
  echo ""
  exit 1
fi
