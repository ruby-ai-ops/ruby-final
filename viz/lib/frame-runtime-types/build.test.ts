...
describe("Frame runtime declaration artifact", () => {
  it("accepts a composed theme in an entry named Frame without shadowing the root", () => {
    expect(
      check({
        "theme.ts": `
import type { FrameTheme } from "@ruby-ai/frame";
export const theme = {
...
import { Document, type DocumentProps } from "@ruby-ai/document/v1";
...
`,
  ...
  it.each([
    [
      'import { Document } from "@ruby-ai/document/v1"; export default () => <Document />',
      2741,
    ],
    [
    ...
    [
      'import { FrameRoot } from "@ruby-ai/frame"; export default () => <FrameRoot theme={{ "--primary": { color: "red" } }}>Report</FrameRoot>',
      2322,
    ],
    [
      'import { Slideshow } from "@ruby-ai/slideshow/v2"; export default () => <Slideshow theme={{ "--primary": true }}>Report</Slideshow>',
      2322,
    ],
    [
