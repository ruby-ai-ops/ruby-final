...
const [vizCSS, uiCSS] = await Promise.all([
  compile("globals.css"),
  compile("ui.css"),
]);
const sample = `
...
try {
  ...
  const stylesheet = await page.addStyleTag({ content: uiCSS });
  ...
  assert.equal(after.portal[0].animationName, "viz-ui-enter");
  ...
  const darkNarrowWithRubyUI = await measure();
  await stylesheet.evaluate((element) => element.remove());
  const darkNarrowWithoutRubyUI = await measure();
  assert.deepEqual(
    darkNarrowWithRubyUI.outside,
    darkNarrowWithoutRubyUI.outside
  );
  assert.deepEqual(
    darkNarrowWithRubyUI.visual,
    darkNarrowWithoutRubyUI.visual
  );

  process.stdout.write(
    "RubyUI styles, portal scope and dark mode passed. Document content follows Frame themes without restyling controls or nested visuals.\n"
  );
} finally {
  await browser.close();
}
