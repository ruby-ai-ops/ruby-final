...
const SCOPE_ROOT = ".viz-ui";
const SCOPE_LIMIT = "[data-viz-ui-slot]";

/**
 * @cc [owner:flvndvd,label:architecture] viz-only-ui-styles
 * RubyUI rules MUST only match inside .viz-ui, excluding data-viz-ui-slot subtrees.
 */
/**
 * @cc [owner:flvndvd,label:architecture] private-ui-css-names
 * Generated custom properties and animations MUST use private names so Frame styles remain unchanged.
 */
export const scopeRubyUI = (css) => {
  ...
  root.walkDecls(({ prop }) => {
    if (prop.startsWith("--")) {
      variables.set(prop, `--viz-ui-${prop.slice(2)}`);
    }
  });
  root.walkAtRules("property", ({ params }) =>
    variables.set(params, `--viz-ui-${params.slice(2)}`)
  );
  root.walkAtRules("keyframes", ({ params }) =>
    animations.set(params, `viz-ui-${params}`)
  );
