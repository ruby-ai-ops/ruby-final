...
} from "@connectors/connectors/ruby_project/lib/mount_path_utils";
import {
...
export async function deleteProjectMountFile({
  ...
  dataSourceConfig: DataSourceConfig;
  projectId: string;
  mountRow: RubyProjectMountFileResource;
}): Promise<void> {
...
async function upsertProjectMountFolder({
  ...
  await upsertDataSourceFolder({
    ...
    title,
    mimeType: INTERNAL_MIME_TYPES.RUBY_PROJECT.MOUNT_FOLDER,
  });
}
...
export async function syncProjectMountFile({
  ...
  const existing =
    await RubyProjectMountFileResource.fetchByConnectorIdAndScopedPath(
      connectorId,
      entry.path
    );
  ...
  if (existing) {
    ...
    await RubyProjectMountFileResource.makeNew({
      connectorId,
      projectId,
      scopedPath: entry.path,
      documentId,
