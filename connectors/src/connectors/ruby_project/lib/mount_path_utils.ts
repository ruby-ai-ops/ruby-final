import { PROJECT_CONTEXT_FOLDER_ID } from "@connectors/connectors/ruby_project/lib/constants";
import { createHash } from "crypto";

// Matches pod-scoped paths from RubyFileSystem: pod-{podId}/...
...
export function getMountDirInternalId(
  ...
  const h = createHash("sha256")
    .update(`ruby-project:${projectId}:dir:${relativeDirPath}`, "utf8")
    .digest("hex")
    .slice(0, 16);
  return `dpd_${h}`;
}
