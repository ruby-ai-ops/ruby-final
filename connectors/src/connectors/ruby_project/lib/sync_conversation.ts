...
export async function deleteConversation({
  ...
  try {
    ...
    const existingConversation =
      await RubyProjectConversationResource.fetchByConnectorIdAndConversationId(
        connectorId,
        conversationId
      );
...
export async function syncConversation({
  ...
  try {
    ...
    await upsertDataSourceFolder({
      ...
      parentId: null,
      title: `Conversations`,
      mimeType: INTERNAL_MIME_TYPES.RUBY_PROJECT.CONVERSATION_FOLDER,
    });
    ...
    if (totalParts === 1) {
      ...
      await upsertDataSourceDocument({
        ...
        mimeType: INTERNAL_MIME_TYPES.RUBY_PROJECT.CONVERSATION_MESSAGES,
        async: true,
        loggerArgs: {
          conversationId: conversation.sId,
          projectId,
    ...
    if (existingConversation) {
      ...
      await RubyProjectConversationResource.makeNew({
        connectorId,
        conversationId: conversation.sId,
        projectId,
        sourceUpdatedAt,
