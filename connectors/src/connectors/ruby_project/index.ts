import {
  launchRubyProjectFullSyncWorkflow,
  launchRubyProjectIncrementalSyncWorkflow,
  signalRubyProjectIncrementalSync,
  stopRubyProjectSyncWorkflow,
} from "@connectors/connectors/ruby_project/temporal/client";
import type {
...
export class RubyProjectConnectorManager extends BaseConnectorManager<null> {
  readonly provider: ConnectorProvider = "ruby_project";
    ...
    if (!connectionId) {
      return new Err(
        new ConnectorManagerError(
          ...
          "connectionId is required for ruby_project connector, it should be the project ID"
        )
      );
    }
  ...
  async update({
    ...
    // No update needed for ruby_project connector
  ...
  async stop({
    ...
    return stopRubyProjectSyncWorkflow({
  ...
  async resume(): Promise<Result<undefined, Error>> {
    ...
    const configuration =
      await RubyProjectConfigurationResource.fetchByConnectorId(
        this.connectorId
      );
    if (!configuration) {
      return new Err(
    ...
    if (!configuration.lastSyncedAt) {
      ...
      const result = await launchRubyProjectFullSyncWorkflow(this.connectorId);
      if (result.isErr()) {
      ...
      const result = await launchRubyProjectIncrementalSyncWorkflow(
        this.connectorId
      );
      if (result.isErr()) {
        return new Err(result.error);
  ...
  async sync({
    ...
    if (fromTs === null) {
      return launchRubyProjectFullSyncWorkflow(this.connectorId);
    } else {
      return launchRubyProjectIncrementalSyncWorkflow(this.connectorId);
    }
  }

  async requestIncrementalSync(): Promise<Result<string, Error>> {
    return signalRubyProjectIncrementalSync(this.connectorId);
  }
  ...
  async setConfigurationKey({
    ...
    // No configuration keys for ruby_project
    return new Ok(undefined);
  }
