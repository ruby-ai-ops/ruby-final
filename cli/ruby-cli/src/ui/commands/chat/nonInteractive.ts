...
export async function resolveSpaceId(
  ...
  const spacesRes = await rubyClient.getSpaces({
...
export async function sendNonInteractiveMessage(
  ...
  if (rubyClientRes.isErr()) {
    const errorMsg = `Failed to get client: ${rubyClientRes.error.message}`;
    if (setError) {
      setError(errorMsg);
      return;
    }
  ...
  if (!rubyClient) {
    const errorMsg = "Authentication required: Run `ruby login` first";
    if (setError) {
      setError(errorMsg);
      return;
    }
  ...
  try {
    ...
    try {
      spaceId = await resolveSpaceId(rubyClient, projectName, projectId);
    } catch (error) {
