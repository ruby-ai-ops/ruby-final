...
import { getRubyClient, resetRubyClient } from "../../utils/rubyClient.js";
...
const Auth: FC<AuthProps> = ({ force = false, apiKey, wId }) => {
  ...
  const effectiveApiKey = apiKey || process.env.RUBY_API_KEY;
  const effectiveWId = wId || process.env.RUBY_WORKSPACE_ID;
  ...
  if (hasApiKey !== hasWId) {
    return (
      <Box flexDirection="column">
        ...
        <Text>
          Set both RUBY_API_KEY and RUBY_WORKSPACE_ID environment variables,
        </Text>
        <Text>
  ...
  const handleWorkspaceSelectionComplete = useCallback(async () => {
    ...
    if (!rubyClient) {
      setError(
        "Failed to get Ruby client. Try authenticating again using `ruby login`."
      );
      return;
    }
    const userInfoRes = await rubyClient.me();
    if (!userInfoRes.isOk()) {
      setError(
        "Failed to get user info. Try authenticating again using `ruby login`."
      );
      return;
    }
  ...
  const handleHeadlessAuth = useCallback(async () => {
    ...
    try {
      ...
      // Reset the ruby client to use the new tokens
      resetRubyClient();
