  ...
  const uploadFiles = async () => {
    ...
    try {
      ...
      if (rubyClientRes.isErr()) {
        onUploadError(`Failed to get client: ${rubyClientRes.error.message}`);
        return;
      }

      const rubyClient = rubyClientRes.value;
      if (!rubyClient) {
        onUploadError("Authentication required. Run `ruby login` first.");
        return;
      }
      ...
      for (const file of files) {
        try {
          ...
          const uploadResult = await rubyClient.uploadFile({
            fileObject,
            fileName: file.name,
            fileSize: file.size,
