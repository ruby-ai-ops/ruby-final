import type { RubyAPI } from "@ruby-ai/client";
...
export class CLIMcpTransport implements Transport {
  ...
  async start(): Promise<void> {
    ...
    this.heartbeatTimer = setInterval(async () => {
      ...
      const res = await this.rubyAPI.heartbeatMCPServer({
        serverId: this.serverId,
      });
      if (res.isErr() || !res.value.success) {
        const reReg = await this.rubyAPI.registerMCPServer({
          serverName: this.serverName,
        });
        if (reReg.isOk()) {
  ...
  async send(message: JSONRPCMessage): Promise<void> {
    ...
    const res = await this.rubyAPI.postMCPResults({
      serverId: this.serverId,
      result: message,
    });
    if (res.isErr()) {
  ...
  private async readLoop(): Promise<void> {
    while (this.running) {
      try {
        const connRes = await this.rubyAPI.getMCPRequestsConnectionDetails({
          serverId: this.serverId!,
