import type { RubyAPI, Result } from "@ruby-ai/client";
...
export const useFileSystemServer = async (
  ...
  const apiKey = await rubyAPI.getApiKey();
  if (apiKey?.startsWith("sk-")) {
    return new Err(
      new Error(
        "File system access requires OAuth authentication. API keys don't support MCP server registration. Please use 'ruby login' to authenticate with OAuth for file system features."
      )
    );
  }
  ...
  const transport = new CLIMcpTransport(rubyAPI, onServerIdReceived, "fs-cli");
