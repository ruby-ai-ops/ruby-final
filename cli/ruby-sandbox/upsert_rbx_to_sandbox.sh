#!/usr/bin/env bash
#
# Build rbx for Linux x86_64 and push it to an e2b sandbox.
#
# Usage:
#   ./upsert_rbx_to_sandbox.sh              # interactive sandbox picker (requires fzf)
#   ./upsert_rbx_to_sandbox.sh <sandbox-id> # push directly to a specific sandbox
#   ./upsert_rbx_to_sandbox.sh --no-build <sandbox-id>  # skip build, just push
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TARGET="x86_64-unknown-linux-musl"
BINARY="$SCRIPT_DIR/target/$TARGET/release/rbx"
REMOTE_PATH="/opt/bin/rbx"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo ""
      echo "Build rbx and push it to a running e2b sandbox."
      echo ""
      echo "Options:"
...
echo "==> Pushing rbx to sandbox $SANDBOX_ID at $REMOTE_PATH..."
...
echo "==> Done! rbx deployed to sandbox $SANDBOX_ID"
