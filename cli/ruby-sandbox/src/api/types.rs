...
/// snake_case `rbx db list` envelope, so local and remote listings print identically.
...
/// `rbx db query` envelope the Bun runner prints for local queries.
#[derive(Debug, Deserialize, Serialize, PartialEq)]
#[serde(rename_all(deserialize = "camelCase"))]
...
/// Front-side Ruby overhead for one `POST .../actions/call` (create + run + early wait).
...
/// Phase breakdown for one `rbx tools` call (POST + poll + optional offload).
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq)]
#[serde(rename_all = "camelCase")]
