...
#[derive(Parser)]
#[command(name = "rbx", version, about = "Ruby sandbox CLI")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Tools {
        ...
        /// Server name as listed by `rbx tools`, or a view id (omit to list all servers)
...
async fn main() {
    ...
    if let Err(error) = run().await {
        ...
        error!(error = %error_chain, "rbx command failed");
 ...
 * it. A subcommand hidden this way MUST still parse, so its runtime guard (or the Ruby API) reports
...
mod tests {
    ...
    fn conversation_requires_frame_target_for_databases() {
        assert!(try_parse_for_owner(SandboxOwner::Conversation, &["rbx", "db", "list"]).is_err());
        assert!(
            try_parse_for_owner(SandboxOwner::Conversation, &["rbx", "db", "query", "chat"])
                .is_err()
        );
        assert!(try_parse_for_owner(
            SandboxOwner::Conversation,
            &["rbx", "db", "list", "--frame", "fil_abc123"]
        )
        .is_ok());
    }
    ...
    fn hidden_commands_still_parse() {
        for (owner, argv) in [
            ...
            (
                SandboxOwner::Conversation,
                &["rbx", "function", "run", "greet"],
            ),
            (
                SandboxOwner::Frame,
                &["rbx", "frame", "call", "fil_abc123", "get-status"],
            ),
            (
                SandboxOwner::Frame,
                &["rbx", "db", "list", "--frame", "fil_abc123"],
    ...
    fn json_flag_parses_before_positionals() {
        let cli = Cli::try_parse_from(["rbx", "tools", "--json", "srv", "tool", "--foo", "bar"])
            .expect("should parse");
    ...
    fn json_flag_after_positionals_is_swallowed_into_args() {
        let cli = Cli::try_parse_from(["rbx", "tools", "srv", "tool", "--foo", "bar", "--json"])
            .expect("should parse");
        let fields = tools_fields(cli);
    ...
    fn tools_without_json_defaults_to_false() {
        let cli = Cli::try_parse_from(["rbx", "tools", "srv", "tool"]).expect("should parse");
        let fields = tools_fields(cli);
        assert!(!fields.json);
    ...
    fn args_json_accepts_stdin_sentinel() {
        let cli = Cli::try_parse_from(["rbx", "tools", "--args-json", "-", "srv", "tool"])
            .expect("should parse");
        let fields = tools_fields(cli);
    ...
    fn function_run_parses() {
        let cli = Cli::try_parse_from(["rbx", "function", "run", "greet"]).expect("parse");
        match cli.command {
            Commands::Function { command } => match command {
                commands::function::FunctionCommand::Run {
                    name,
    ...
    fn function_materialize_archive_parses() {
        let cli = Cli::try_parse_from(["rbx", "function", "materialize-archive"]).expect("parse");
        match cli.command {
            Commands::Function { command } => match command {
                commands::function::FunctionCommand::MaterializeArchive => {}
    ...
    fn function_get_parses() {
        let cli = Cli::try_parse_from(["rbx", "function", "get", "greet"]).expect("parse");
        match cli.command {
            Commands::Function { command } => match command {
    ...
    #[test]
    fn db_schema_parses() {
        let cli = Cli::try_parse_from(["rbx", "db", "schema", "chat", "/tmp/out/chat.db.ts"])
            .expect("parse");
        match cli.command {
            Commands::Db { command } => match command {
                commands::db::DbCommand::Schema { name, out_schema } => {
    ...
    #[test]
    fn db_list_parses() {
        let cli = Cli::try_parse_from(["rbx", "db", "list"]).expect("parse");
        match cli.command {
            Commands::Db { command } => match command {
    ...
    fn db_list_with_frame_parses() {
        let cli =
            Cli::try_parse_from(["rbx", "db", "list", "--frame", "fil_abc123"]).expect("parse");
        match cli.command {
            Commands::Db { command } => match command {
                commands::db::DbCommand::List { frame } => {
    ...
    #[test]
    fn db_query_parses() {
        let cli = Cli::try_parse_from(["rbx", "db", "query", "chat"]).expect("parse");
        match cli.command {
            Commands::Db { command } => match command {
                commands::db::DbCommand::Query { name, frame } => {
                    assert_eq!(name, "chat");
    ...
    fn db_query_with_frame_parses() {
        let cli = Cli::try_parse_from(["rbx", "db", "query", "chat", "--frame", "fil_abc123"])
            .expect("parse");
        match cli.command {
            Commands::Db { command } => match command {
                commands::db::DbCommand::Query { name, frame } => {
    ...
    fn db_reconcile_requires_schema_file() {
        assert!(Cli::try_parse_from(["rbx", "db", "reconcile", "chat"]).is_err());
    }
