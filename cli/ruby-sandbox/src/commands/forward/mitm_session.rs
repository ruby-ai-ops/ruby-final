...
mod tests {
    ...
    async fn mitm_session_copies_decrypted_bytes_unchanged() -> Result<()> {
        ...
        let (agent_client_io, agent_rbx_io) = tokio::io::duplex(4096);
        let (rbx_proxy_io, upstream_server_io) = tokio::io::duplex(4096);
        ...
        let agent_task = tokio::spawn(async move {
            ...
            let mut tls = agent_connector
                ...
                .await
                .context("test agent failed to connect to rbx MITM")?;
            ensure!(
                tls.get_ref().1.alpn_protocol() == Some(HTTP_1_1_ALPN),
        ...
        run_mitm_session_with_proxy_opener(
            ...
            sni,
            agent_rbx_io,
            single_proxy_opener(rbx_proxy_io),
        )
        .await?;
    ...
    async fn mitm_session_substitutes_http_headers() -> Result<()> {
        ...
        let (agent_client_io, agent_rbx_io) = tokio::io::duplex(16 * 1024);
        let (rbx_proxy_io, upstream_server_io) = tokio::io::duplex(16 * 1024);
    ...
    async fn mitm_session_does_not_splice_client_frames_when_upgrade_rejected() -> Result<()> {
        ...
        let upstream_task = tokio::spawn(async move {
            ...
            // After the rejection, rbx must shut down the upstream write
        ...
        let agent_task = tokio::spawn(async move {
            ...
            // The agent attempts to send post-upgrade frames; rbx must drop
    ...
    // SNI-miss path: rbx does not terminate, just splices. The agent's TLS
    // client must see the upstream's real cert chain (not the rbx CA),
    ...
    async fn splice_session_preserves_upstream_chain() -> Result<()> {
        ...
        let (agent_client_io, mut agent_rbx_io) = tokio::io::duplex(4096);
        let (mut rbx_proxy_io, upstream_server_io) = tokio::io::duplex(4096);
        ...
        // Trust ONLY the upstream's real CA. If rbx silently terminated and
        ...
        let agent_task = tokio::spawn(async move {
            ...
            let mut tls = agent_connector
                .connect(server_name, agent_client_io)
                .await
                .context("test agent failed to connect through rbx splice")?;
            ...
            assert_ne!(
                ...
                "splice path leaked the rbx MITM CA into the agent's TLS view"
            );
        ...
        tokio::io::copy_bidirectional(&mut agent_rbx_io, &mut rbx_proxy_io)
            .await
            .ok();
        agent_task.await.context("test agent task panicked")??;
        upstream_task
    ...
    async fn plain_http_session_denies_fronted_host_on_keep_alive() -> Result<()> {
        ...
        // rbx must reject a second request that swaps in another host rather
        ...
        let (mut agent_io, rbx_client_io) = tokio::io::duplex(16 * 1024);
        let (rbx_upstream_io, mut upstream_io) = tokio::io::duplex(16 * 1024);
        let session_task = tokio::spawn(async move {
            run_plain_http_session(&runtime, domain, rbx_client_io, rbx_upstream_io).await
        });
