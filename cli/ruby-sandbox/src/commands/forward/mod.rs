...
pub async fn cmd_forward(args: ForwardArgs) -> Result<()> {
    ...
    // The secrets file is intentionally loaded once at rbx startup. Front
    // propagates changes in Phase 1 by rewriting the file and restarting rbx
...
fn build_tls_connector_with_alpn(
    ...
    // On a wake/restart the trust-bundle installer has already added rbx's
    // own CA at /etc/ssl/certs/ruby-egress.pem, so the restarted rbx will
    // load its own CA into this outbound trust set. The "rbx must not trust
    ...
    // than filtered out here. If that assumption ever weakens, switch rbx
