...
// cast from u128 cannot overflow for any realistic rbx lifetime.
...
mod tests {
    ...
    async fn outbound_h2_settings_disable_push() -> Result<()> {
        let (rbx_io, mut upstream_io) = tokio::io::duplex(64 * 1024);
        ...
        let (pooled, lease) = open_h2_connection(Box::new(rbx_io) as BoxedAsyncReadWrite).await?;
