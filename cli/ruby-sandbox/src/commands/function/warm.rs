//! Warm-path client for `rbx function run`.
//!
//! Fast invocations talk to one publication-scoped Bun worker (keyed by
//! publication id under `$HOME/.ruby-fn/publications/`). Seed or the first
...
/// (front) enforces the real invocation timeout by killing rbx. This only
...
fn write_bundle_cache_entry(bytes: &[u8], sha256: &str) -> bool {
    ...
    // Write-then-rename so a concurrent rbx never observes (or imports) a
    // half-written bundle.
...
pub async fn try_warm_run(name: &str, input: &str) -> WarmRun {
    ...
    // execs rbx as agent-proxied. A root rbx stays cold — a root client
...
"#
    ...
    async fn warm_cycle_end_to_end() {
        ...
        std::env::set_var("RUBY_FUNCTIONS_DIR", &functions_dir);
