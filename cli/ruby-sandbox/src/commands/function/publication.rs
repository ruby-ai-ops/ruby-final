...
/// Publication id from `$RUBY_FUNCTIONS_DIR`'s parent basename. Pure path math
...
pub async fn ensure_publication_worker() -> Option<PathBuf> {
    ...
    let functions_dir = std::env::var("RUBY_FUNCTIONS_DIR").ok()?;
...
/// Entry for `rbx function materialize-archive`: start the publication
...
pub async fn materialize_publication() -> Result<(), String> {
    let dir = std::env::var("RUBY_FUNCTIONS_DIR")
        .map_err(|_| "RUBY_FUNCTIONS_DIR is not set".to_string())?;
    if dir.is_empty() {
        return Err("RUBY_FUNCTIONS_DIR is empty".to_string());
    }
