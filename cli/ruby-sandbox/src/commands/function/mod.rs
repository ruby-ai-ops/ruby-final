...
/// committed: `bun run build` in `functions-runner/` produces it before rbx
/// compiles (see build.rs). Embedded so `rbx` is a single binary;
...
/// `rbx healthcheck`'s nftables rules force through the egress proxy). Untrusted
...
pub enum FunctionCommand {
    ...
    Run {
        ...
        /// rbx is pinned by release, so a binary that rejected it would break
        /// every invocation the moment RBX_CLI_VERSION moved. Remove it with
...
/// Whether `rbx` is running privileged (effective uid 0).
///
...
/// like agent code. When `rbx` is already unprivileged (local dev), there is
...
/// downgraded to the `agent-proxied` user whenever `rbx` runs privileged: the
/// child is launched via `runuser`, which resolves that user's uid/gid/
/// supplementary groups and drops privileges before exec — so `rbx` needs no
...
pub(crate) async fn spawn_function_at(
    ...
    // groups) and execs `bun` — no privileged code in rbx. Otherwise run `bun`
    ...
    // No env_clear: the child inherits rbx's env verbatim, so per-exec vars
    // front sets (e.g. RUBY_POD_DATABASES_DIR, read by `@ruby-ai/pod`) flow
    // through without rbx naming each one — pinned by the inheritance tests.
...
/// `runuser` whenever `rbx` runs privileged, so its egress is forced through
...
pub(crate) async fn spawn_build(src: &Path, out_bundle: &Path, out_schema: &Path) -> Result<i32> {
    ...
    // top-level code) and inherits rbx's env like function runs do, so a
...
/// Dropped to `agent-proxied` via `runuser` whenever rbx runs privileged, NODE_PATH
...
pub(crate) async fn spawn_runner(
    ...
    let as_agent = running_as_root();
    if as_agent {
        // The dropped child must read the runner rbx wrote as root (0600).
        set_mode(&runner, 0o644)
            .map_err(|e| emit_error(anyhow!("failed to prepare runner: {e}")))?;
    }
...
/// `functions.tar` (materialized under `$HOME/.ruby-fn/archives/`).
/// `$RUBY_FUNCTIONS_DIR` only locates the publication: its parent holds the
...
mod tests {
    ...
    // Run `f` with RUBY_FUNCTIONS_DIR pointing at `.../<pub-id>/functions`
    ...
    /// reads it in the bun child, and rbx passes it along by plain env
...
"#;
    ...
    /// End-to-end pin of the env contract's rbx hop, through the production
    /// `rbx function get` path: a var present in rbx's process env (front
    ...
    async fn function_get_bun_child_inherits_pod_databases_dir() {
        ...
        // A var set on rbx's own process reaches the child by inheritance.
        ...
        // Absent env var: no rbx-side default, the child sees it unset.
    ...
    /// Same pin for the `rbx function build` path: schema extraction imports
