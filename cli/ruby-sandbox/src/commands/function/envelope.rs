...
pub enum ResolveKind {
    /// Local `~/.ruby-fn/bundles/<sha>.js` hit.
    Cache,
    /// Local archive extract under `~/.ruby-fn/archives/<publication_id>/`.
    Extract,
}
...
pub struct TimingsMs {
    pub total: u64,
    pub runner: u64,
    /// Additive: absent on envelopes from older rbx versions, and front
