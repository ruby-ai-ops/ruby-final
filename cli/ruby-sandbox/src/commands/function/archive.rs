...
//! `$HOME/.ruby-fn/archives/<publication_id>/` gives the publication worker
...
/// `$HOME/.ruby-fn/archives/<publication_id>/` and eagerly populate the
...
/// `$HOME/.ruby-fn/archives` — local extract trees keyed by publication id.
...
fn materialize_archive(archive_path: &Path, publication_id: &str) -> Option<PathBuf> {
    ...
    match std::fs::rename(&tmp_dir, &extract_dir) {
        Ok(()) => Some(extract_dir),
        Err(e) if e.kind() == ErrorKind::AlreadyExists => {
            // Lost the race to another rbx; use the winner's tree.
            let _ = std::fs::remove_dir_all(&tmp_dir);
            if extract_dir.join(COMPLETE_MARKER).is_file() {
                Some(extract_dir)
            } else {
