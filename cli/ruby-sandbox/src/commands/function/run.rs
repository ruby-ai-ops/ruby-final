    ...
    match serde_json::from_str::<serde_json::Value>(line) {
        ...
        Err(_) => {
            ...
            if line.starts_with('{') || line.starts_with('[') {
                ...
                eprintln!("rbx: truncated function output (read {bytes_read} bytes): {snippet}");
                return (
...
/// Lift `_rubyTimingsMs` from the Bun child's stdout JSON into the outer
...
fn merge_runner_phase_timings(
    ...
    let Some(sidecar) = obj.remove(RUBY_RUNNER_TIMINGS_MS_KEY) else {
        return timings_ms;
    };
    ...
    fn stdout_result_lifts_runner_phase_timings_sidecar() {
        let (envelope, code) = stdout_result(
            Ok((
                0,
                Some(
                    r#"{"ok":true,"output":1,"_rubyTimingsMs":{"import":40,"handler":5,"tools":{"total":12,"count":1,"calls":[{"server":"gmail","tool":"get_messages","post":3,"poll":9,"total":12}]}}}"#
                        .to_string(),
                ),
            )),
            timings(12, 8),
