...
use crate::api::{DatabaseEntry, RubyApiClient};
...
fn enumerate_databases(dir: &Path) -> std::io::Result<Vec<DatabaseEntry>> {
    ...
    for entry in entries.flatten() {
        ...
        // must not have rbx report (or stat) a foreign file. DirEntry::file_type/metadata
...
mod tests {
    ...
    fn missing_dir_is_an_empty_list() -> Result<()> {
        let databases = enumerate_databases(Path::new("/nonexistent/rbx-db-list-test"))?;
