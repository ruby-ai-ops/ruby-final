//! `rbx db` database subcommands (reconcile/schema/list/query).
//!
//! Databases are sandbox-owned SQLite files `{name}.db` under `$RUBY_POD_DATABASES_DIR`
...
//! `rbx function`: dropped to `agent-proxied` via `runuser` whenever rbx runs as
