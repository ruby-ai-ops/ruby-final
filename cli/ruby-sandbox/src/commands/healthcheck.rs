...
pub struct HealthcheckArgs {
    ...
    #[arg(long, default_value = "/etc/ruby/ca-bundle.pem")]
    ca_bundle: PathBuf,
    /// Marker written after the merged MITM trust bundle is installed
    #[arg(long, default_value = "/etc/ruby/.ca-bundle.merged")]
...
fn nft_ruleset(family: &str) -> Option<String> {
    ...
    let output = match Command::new("nft")
        .args(["-n", "list", "table", family, "ruby-egress"])
        .output()
    {
        Ok(output) => output,
        Err(error) => {
    ...
    if !output.status.success() {
        ...
        warn!(
            ...
            stderr = %stderr,
            family = family,
            "nft list table ruby-egress exited non-zero"
        );
        return None;
    }
