...
const CACHE_MARKER: &str = ".ruby-filesystem-cache-v1";
...
const CACHE_MARKER_TEMPORARY: &str = ".ruby-filesystem-cache-v1.tmp";
const CACHE_MARKER_CONTENT: &[u8] = b"Ruby filesystem staging directory\n";
...
fn open_staged_file(path: &Path, flags: i32) -> io::Result<File> {
    ...
    options
        ...
        // not this private cache fd, and are handled or rejected by RubyFuse.
        .custom_flags(libc::O_NOFOLLOW | libc::O_CLOEXEC);
    options.open(path)
}
