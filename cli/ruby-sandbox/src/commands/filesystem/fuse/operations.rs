...
//!
//! Each method validates the Linux request, calls `RubyFuse` or `FileStore`,
...
impl Filesystem for RubyFuse {
    ...
    fn access(&self, _request: &Request, inode: INodeNo, mask: AccessFlags, reply: ReplyEmpty) {
        ...
        self.spawn_remote("access", RemoteWork::Metadata, move |rejected| {
            ...
            let result = filesystem.node(inode).and_then(|node| {
                ...
                // mode rule Ruby stores is whether a file may be executed.
    ...
    fn setattr(
        ...
        gid: Option<u32>,
        size: Option<u64>,
        // Ruby records when a file's contents last changed and stores no time
        ...
        // Ruby stores no owner, and every file reports the daemon's own user.
    ...
    fn readlink(&self, _request: &Request, inode: INodeNo, reply: ReplyData) {
        ...
        // carry a Ruby identifier in their name. The daemon builds them when it
