#!/usr/bin/env bun
// End-to-end smoke tests for `rbx forward` + the live egress-proxy.
//
// Runs inside the e2e Docker image as root. For each case: mints a JWT,
// writes it to the forwarder's token file, spawns `rbx forward`, then drops
...
//   RBX_LISTEN_ADDR          default 127.0.0.1:9990
//   RBX_TOKEN_FILE           default /tmp/egress-token
//   RBX_DENY_LOG             default /tmp/ruby-egress-denied.log
...
const LISTEN_ADDR = process.env.RBX_LISTEN_ADDR ?? "127.0.0.1:9990";
const TOKEN_FILE = process.env.RBX_TOKEN_FILE ?? "/tmp/egress-token";
const DENY_LOG_PATH =
  process.env.RBX_DENY_LOG ?? "/tmp/ruby-egress-denied.log";
...
async function waitForListen(addr: string, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`rbx forward did not bind ${addr} within ${timeoutMs}ms`);
}
