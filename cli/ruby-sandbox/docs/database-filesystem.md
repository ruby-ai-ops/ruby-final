...
1. Linux sends a FUSE operation to `rbx`.
2. `rbx` sends one typed request to
...
1. Front returns a short-lived signed GCS download URL for the node's current
   blob.
2. `rbx` downloads it into `/run/ruby-filesystem/staging` on first open.
...
3. `rbx` uploads directly to the signed GCS URL.
...
outside the timed phases, injects the local Linux `rbx` into the two database
