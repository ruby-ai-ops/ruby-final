#!/usr/bin/env bun
// Embedded runner for `rbx function` and `rbx db`. Subcommands:
...
// it — every replication sync fails with SQLITE_READONLY, forever. `rbx db
...
async function runHandler(handlerPath: string): Promise<number> {
  ...
  // Phase timings ride as a sidecar rbx lifts into the outer envelope.
