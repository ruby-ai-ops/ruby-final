// Reads the invocation environment through @ruby-ai/pod's accessor, imported
...
// Symbol.for registry, exactly like the vendored @ruby-ai/pod does in the
...
import { invocationEnv } from "../../pod/context.ts";

export default {
  async fetch(req: Request): Promise<Response> {
    const url = new URL(req.url);
    const delayMs = Number(url.searchParams.get("delayMs") ?? "0");
    const before = invocationEnv("WARM_TEST_MARKER") ?? null;
    if (delayMs > 0) {
      await new Promise((resolve) => setTimeout(resolve, delayMs));
    }
    const after = invocationEnv("WARM_TEST_MARKER") ?? null;
    return Response.json({
      before,
      after,
      identity: invocationEnv("RUBY_POD_USER_IDENTITY") ?? null,
    });
  },
};
