...
// `functionsDir`, then writes an optional ready marker so rbx can wait for
...
// Mirrors rbx's function-name validation: the name feeds a directory scan.
 ...
 * the same contract as rbx's archive extract resolve: exactly one file in the
...
export function listFunctionNames(functionsDir: string): string[] {
  ...
  for (const entry of entries) {
    ...
    // Same charset as rbx `is_valid_name`.
