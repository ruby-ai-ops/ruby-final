...
// the handler's code. The reader (rbx, or front through it) then sees a
...
/** Sidecar key the cold Bun child stamps on its stdout JSON; rbx strips it
 * into the outer envelope `timingsMs` so front's strict outcome schema never
 * sees it. */
export const RUBY_RUNNER_TIMINGS_MS_KEY = "_rubyTimingsMs";

export type DeliverableWithRunnerTimings = DeliverableOutput & {
  [RUBY_RUNNER_TIMINGS_MS_KEY]?: InvokePhaseTimingsMs;
};
...
export function attachRunnerTimings(
  ...
  return { ...out, [RUBY_RUNNER_TIMINGS_MS_KEY]: timingsMs };
}
