...
impl Forwarder {
    async fn spawn(
        ...
        let mut child = Command::new(env!("CARGO_BIN_EXE_rbx"))
            .arg("forward")
            .arg("--token-file")
            .arg(&token_file)
            .arg("--proxy-addr")
            ...
            .spawn()
            .context("failed to spawn rbx forward")?;
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                if line.contains("starting rbx forwarder") {
        ...
        tokio::time::timeout(Duration::from_secs(5), ready_rx)
            .await
            .context("timed out waiting for rbx forward readiness")?
            .context("rbx forward stderr closed before readiness")?;
...
impl TestCa {
    fn new() -> Result<Self> {
        ...
        dn.push(DnType::CommonName, "rbx local e2e CA");
