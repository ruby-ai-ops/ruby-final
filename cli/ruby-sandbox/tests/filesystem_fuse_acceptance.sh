#!/usr/bin/env bash
# Run inside a Linux sandbox after the Ruby filesystem is mounted at /files.
set -euo pipefail

MOUNTPOINT="${MOUNTPOINT:-/files}"
RUN_ID="ruby-fs-acceptance-$$-$(date +%s)"
...
# The short names are links to the roots that carry a Ruby identifier. Linux
...
export RUBY_TRUNCATE_PATH="$CONVERSATION_DIR/js-truncate.txt"
printf seed >"$RUBY_TRUNCATE_PATH"
"$JS_RUNTIME" -e '
  const fs = require("node:fs");
  const fd = fs.openSync(process.env.RUBY_TRUNCATE_PATH, fs.constants.O_WRONLY | fs.constants.O_TRUNC);
  fs.writeSync(fd, Buffer.from("javascript"));
  fs.fsyncSync(fd);
  fs.fsyncSync(fd);
  fs.closeSync(fd);
'
test "$(cat "$RUBY_TRUNCATE_PATH")" = javascript
...
export RUBY_FLAG_PATH="$CONVERSATION_DIR/open-flags.txt"
printf stable >"$RUBY_FLAG_PATH"
"$JS_RUNTIME" -e '
  const fs = require("node:fs");
  const cases = [
  ...
  for (const [flags, expected] of cases) {
    try {
      fs.openSync(process.env.RUBY_FLAG_PATH, flags);
...
'
test "$(cat "$RUBY_FLAG_PATH")" = stable
...
export RUBY_DIRTY_UNLINK_PATH="$CONVERSATION_DIR/dirty-unlink.txt"
export RUBY_DIRTY_DESTINATION_PATH="$CONVERSATION_DIR/dirty-destination.txt"
export RUBY_DIRTY_SOURCE_PATH="$CONVERSATION_DIR/dirty-source.txt"
printf committed >"$RUBY_DIRTY_UNLINK_PATH"
printf old-destination >"$RUBY_DIRTY_DESTINATION_PATH"
printf new-source >"$RUBY_DIRTY_SOURCE_PATH"
"$JS_RUNTIME" -e '
  const fs = require("node:fs");

  const unlinked = fs.openSync(process.env.RUBY_DIRTY_UNLINK_PATH, "r+");
  fs.writeSync(unlinked, Buffer.from("dirty"), 0, 5, 0);
  fs.unlinkSync(process.env.RUBY_DIRTY_UNLINK_PATH);
  fs.fsyncSync(unlinked);
  fs.closeSync(unlinked);

  const replaced = fs.openSync(process.env.RUBY_DIRTY_DESTINATION_PATH, "r+");
  fs.writeSync(replaced, Buffer.from("dirty"), 0, 5, 0);
  fs.renameSync(
    process.env.RUBY_DIRTY_SOURCE_PATH,
    process.env.RUBY_DIRTY_DESTINATION_PATH,
  );
  fs.fsyncSync(replaced);
  fs.closeSync(replaced);
'
test ! -e "$RUBY_DIRTY_UNLINK_PATH"
test "$(cat "$RUBY_DIRTY_DESTINATION_PATH")" = new-source
...
# Ruby stores executable bits for sandbox tools. Read/write bits stay at the
...
# Ruby records when contents last changed and ignores a time chosen by the
...
test "$("$EXECUTABLE")" = ruby-executable-ok
...
# Ruby saves one writer's version at a time, so a second writer is refused
# instead of one of the two versions being lost.
export RUBY_SINGLE_WRITER_PATH="$CONVERSATION_DIR/single-writer.txt"
printf seed >"$RUBY_SINGLE_WRITER_PATH"
"$JS_RUNTIME" -e '
  const fs = require("node:fs");
  const first = fs.openSync(process.env.RUBY_SINGLE_WRITER_PATH, "r+");
  try {
    fs.openSync(process.env.RUBY_SINGLE_WRITER_PATH, "r+");
    throw new Error("a second writer was accepted");
  } catch (error) {
    if (error.code !== "EBUSY") throw error;
  }
  // Reading the same file alongside the writer stays allowed.
  fs.closeSync(fs.openSync(process.env.RUBY_SINGLE_WRITER_PATH, "r"));
  fs.closeSync(first);
  fs.closeSync(fs.openSync(process.env.RUBY_SINGLE_WRITER_PATH, "r+"));
'
...
export RUBY_CONCURRENT_PATH="$CONVERSATION_DIR/write-during-save.txt"
export RUBY_JS_RUNTIME="$JS_RUNTIME"
"$JS_RUNTIME" -e '
  const fs = require("node:fs");
  const { spawn, spawnSync } = require("node:child_process");

  const runtime = process.env.RUBY_JS_RUNTIME;
  const chunk = Buffer.alloc(1024 * 1024, "a");
  const fd = fs.openSync(process.env.RUBY_CONCURRENT_PATH, "w");
...
'
test "$(stat -c %s "$RUBY_CONCURRENT_PATH")" = "$((16 * 1024 * 1024))"

echo "Ruby filesystem acceptance cases passed"
