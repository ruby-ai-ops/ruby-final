 ...
 * database at `${RUBY_SANDBOX_DATABASES_DIR}/{prefix}{name}.db`. Names may be
 ...
 * `rbx function run`, which forwards it here — no layer below front carries
 ...
 * passed per-exec through rbx — no copy of the number lives below front.
  ...
  if (dir === undefined || dir.length === 0) {
    throw new SandboxDatabaseError(
      `${SANDBOX_DATABASES_DIR_ENV} is not set: the databases directory is ` +
        `chosen by front and passed through rbx function run, so db() only ` +
  ...
  if (raw === undefined || raw.length === 0) {
    throw new SandboxDatabaseError(
      `${SANDBOX_DATABASE_MAX_SIZE_BYTES_ENV} is not set: the per-database size ` +
        `quota is chosen by front and passed through rbx function run, so ` +
 ...
 *   in functions launched by `rbx function run`.
