...
/**
 * A fake rbx executable: a shell script that records its argv, stdin, and
...
function makeFakeRbx({
  ...
  const dir = mkdtempSync(join(tmpdir(), "fake-rbx-"));
  const path = join(dir, "rbx");
  writeFileSync(
    path,
    [
      "#!/bin/sh",
...
function recorded(fake: FakeRbx, name: string): string {
...
describe("tools.call", () => {
  test("spawns rbx with the delegation argv and returns the parsed result", async () => {
    const fake = makeFakeRbx({ stdout: resultJson() });
  ...
  test("a tool-level error resolves with isError true instead of throwing", async () => {
    // rbx exits 1 when the tool reported an error, with the result payload
    // still on stdout.
    const fake = makeFakeRbx({
      stdout: resultJson({
...
describe("environment handling", () => {
  test("throws missing_env without RUBY_API_URL", async () => {
    ...
    const error = await expectToolCallError(
      callThroughFake(fake, {
        env: { [TOOLS_SANDBOX_TOKEN_ENV]: TOKEN, [RBX_PATH_ENV]: fake.path },
      })
    );
  ...
  test("throws missing_env without RUBY_SANDBOX_TOKEN", async () => {
    ...
    const error = await expectToolCallError(
      callThroughFake(fake, {
        env: { [TOOLS_API_URL_ENV]: BASE_URL, [RBX_PATH_ENV]: fake.path },
      })
    );
    expect(error.code).toBe("missing_env");
...
describe("error envelope classification", () => {
  ...
  test("an envelope code this client does not know degrades to unknown", async () => {
    // The envelope contract is append-only: a newer rbx may emit codes this
  ...
  test("rbx-side validation failures arrive as unknown with the message", async () => {
    // e.g. a typo'd server name: rbx bails before any tool call and emits
...
describe("process failures", () => {
  test("an unparseable stdout on exit 0 is invalid_response", async () => {
    const fake = makeFakeRbx({ stdout: "not json at all" });
    const error = await expectToolCallError(callThroughFake(fake));
  ...
  test("a missing rbx binary is exec_error naming the path", async () => {
    const error = await expectToolCallError(
      runWithInvocationEnv(
        {
          [TOOLS_API_URL_ENV]: BASE_URL,
          [TOOLS_SANDBOX_TOKEN_ENV]: TOKEN,
          [RBX_PATH_ENV]: "/nonexistent/rbx-test-12345",
        },
    ...
    expect(error.code).toBe("exec_error");
    expect(error.retryable).toBe(false);
    expect(error.message).toContain("/nonexistent/rbx-test-12345");
  });

  test("kills the child and throws timeout past timeoutMs", async () => {
    const fake = makeFakeRbx({ stdout: resultJson(), sleepSeconds: 30 });
    const error = await expectToolCallError(
  ...
  test("throws timeout when an exited child's pipe never reaches EOF", async () => {
    ...
    const fake = makeFakeRbx({ stdout: resultJson(), holdPipeSeconds: 30 });
    const error = await expectToolCallError(
      callThroughFake(fake, { timeoutMs: 150 })
    );
    expect(error.code).toBe("timeout");
