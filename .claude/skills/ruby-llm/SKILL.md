...
> Ruby-managed EU hosting is sold to credit-priced plans and to workspaces carrying
> `use_vertex_for_supported_models`; that rule lives once, in
> `front/lib/llms/utils/endpoint_filters.ts`, and every `*_eu_agent_platform.ts` ruby wrapper
> assigns it verbatim:
>
> ```ts
...
| `front/lib/llms/stream/endpoints/{...}.ts` | One thin ruby wrapper per endpoint extending the `model_constructors` class via the ruby mixin; call `defineRubyStreamEndpoint(...)`. This is where `endpointFilter` lives — `EU_AGENT_PLATFORM_ENDPOINT_FILTER` on every `*_eu_agent_platform.ts`, `{}` otherwise. |
...
>   `ruby-*` global agents, and `MODEL_STREAMS` candidates in
...
- [ ] `USED_MODEL_CONFIGS` holds at most two versions of the family; every model evicted by
      ...
      ladder (`ORDERED_*_MODEL_CONFIGS`, `getFastModelConfig`, `ruby-*` global agents,
      `MODEL_STREAMS`)
