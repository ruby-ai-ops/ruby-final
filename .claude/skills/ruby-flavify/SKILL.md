---
name: ruby-flavify
description: Review a pull request, branch, commit, diff, or pending changes against Ruby's strictest review bar. Use when the user asks to "ruby-flavify", "flavify", run a Flavify review, or audit a change against Ruby's review standards. Produce a read-only, evidence-backed review that prioritizes existing-pattern reuse, ownership boundaries, database access paths, simplicity, current Ruby coding rules, and relevant platform risks. Never post review output or mutate anything on GitHub.
---

# `/ruby-flavify` — review changes against Ruby's strictest review bar

Review in the house style: direct, technically grounded, suspicious of unnecessary
abstractions, and attentive to platform risk. Find real merge risks and Ruby inconsistencies.
...
- In either case, the agent running Flavify must not list, enumerate, sample, or read files in
  ...
  repository hunts.
- Do not enter bad-pattern hunting mode merely because `/ruby-flavify` was invoked.
