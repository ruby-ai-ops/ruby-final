...
Use an existing similar icon temporarily, then request the final icon from design/RubyUI and
update the `icon` field once available.

When wiring up the icon:

1. Add the SVG to `ui/src/logo/platforms/` if it doesn't already exist, and re-export it
   from `ui/src/logo/platforms/index.ts`.
2. Always check `ui/src/logo/platforms/registry.ts` and add the logo to `PLATFORM_LOGOS`.
...
3. **Marketing must be redeployed for the new icon to appear on the public integrations page —
   explicitly tell the engineer.** The change lands in the ui bundle that marketing builds
...
- the icon is present in `PLATFORM_LOGOS` in `ui/src/logo/platforms/registry.ts`
