...
} from "@app/types/api/admin/production_checks";
...
export function useAdminProductionChecksAllCells() {
  ...
  useEffect(() => {
    ...
    const run = async (showLoading: boolean) => {
      ...
      const settled =
        await fetchAdminFromAllCells<GetProductionChecksResponseBody>({
          cells,
          path: "/api/admin/production-checks",
        });
...
export function useAdminCheckHistoryForCell(
  cellUrl: string | null,
  checkName: string,
  enabled: boolean
) {
  ...
  const { data, error, mutate } = useSWR(
    enabled && cellUrl
      ? `${cellUrl}/api/admin/production-checks/${encodeURIComponent(checkName)}/history`
      : null,
    checkHistoryFetcher
  );
  ...
  const runCheck = async (
    ...
    try {
      const res = await clientFetch(
        `${cellUrl}/api/admin/production-checks/run`,
        {
          method: "POST",
