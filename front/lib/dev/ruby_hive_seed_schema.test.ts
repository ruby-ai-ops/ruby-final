...
/**
 * Schema validation test for ruby-hive SQL seed.
 *
 * This test validates that the raw SQL used by ruby-hive to seed dev environments
 * is compatible with the current database schema. If this test fails, it means
 * the schema has drifted and ruby_hive_seed.sql needs to be updated.
 *
 * The SQL is defined in front/lib/dev/ruby_hive_seed.sql (single source of truth).
 */
...
const SQL_FILE_PATH = path.join(__dirname, "ruby_hive_seed.sql");

// Error message with prompt for Claude Code when schema drifts
const SCHEMA_DRIFT_ERROR = `
================================================================================
RUBY-HIVE SEED SQL SCHEMA DRIFT DETECTED
================================================================================

The database schema has changed and the ruby-hive seed SQL is no longer compatible.
Please update front/lib/dev/ruby_hive_seed.sql to match the current schema.

To fix this, run Claude Code with the following prompt:

---
The ruby-hive seed SQL schema validation test is failing. Please:
...
3. Update front/lib/dev/ruby_hive_seed.sql to match the current schema:
...
describe("ruby-hive seed SQL schema compatibility", () => {
  ...
  it("validates that ruby-hive seed SQL is compatible with current schema", async () => {
    ...
    try {
      const result = await frontSequelize.query(seedSql, {
        replacements: {
          userId: testUserId,
          username: "test_ruby_hive_seed",
          email: `test_ruby_hive_${Date.now()}@example.com`,
          name: "Test Ruby Hive Seed",
          firstName: "Test",
          lastName: null,
          workOSUserId: null,
          provider: null,
          ...
          workspaceName: "Test Ruby Hive Workspace",
      ...
      const [users] = await frontSequelize.query(
        `SELECT id, "sId", username, "isRubySuperUser" FROM users WHERE id = :userId`,
      ...
      console.error(error);
      throw new Error(
        `Ruby-hive seed SQL schema validation failed. See error output above for details and fix instructions.`
      );
    }
  });
});
