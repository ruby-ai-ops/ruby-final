...
} from "@app/types/admin/credit_usage_configuration";
import { Err, Ok } from "@app/types/shared/result";

export { MAX_AWU_USAGE_CAP_CREDITS } from "@app/types/admin/credit_usage_configuration";

const ADMIN_AUDIT_CONTEXT = { location: "admin" };
  ...
  execute: async (auth, _, args) => {
    ...
    if (defaultPoolCapCredits !== currentPoolCapCredits) {
      const poolResult = await setDefaultUserSpendLimit(auth, {
        ...
        auditContext: ADMIN_AUDIT_CONTEXT,
      });
      if (poolResult.isErr()) {
