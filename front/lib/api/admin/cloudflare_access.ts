...
import { mapAccessGroupNamesToAdminRoles } from "@app/lib/admin/roles";
  ...
  try {
    ...
    if (!email || !sub) {
      logger.warn(
        ...
        "[admin] Cloudflare Access JWT missing email or sub claim"
      );
      return null;
    }
    ...
    logger.warn(
      ...
      "[admin] Cloudflare Access JWT verification failed"
    );
    return null;
  }
}

export async function getAdminRolesForUserViaCloudflareAccess(
  accessToken: string
): Promise<AdminRole[]> {
  const accessConfig = getCloudflareAccessConfig();
  if (!accessConfig) {
  ...
  if (response.ok) {
    ...
    return mapAccessGroupNamesToAdminRoles(
