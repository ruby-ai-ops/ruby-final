...
// Type casts to enable cross-workspace queries for admin super-admin.
...
interface ListGlobalAgentFeedbacksParams {
  /** If false (the default in the admin UI), feedbacks with empty content are filtered out. */
...
/**
 * Aggregate global-agent feedback rows across all workspaces for the admin
...
export async function listGlobalAgentFeedbacks({
  ...
  // Counted across all workspaces for the admin super-admin review tool. The
  ...
  // WORKSPACE_ISOLATION_BYPASS: Admin super-admin query across all workspaces
