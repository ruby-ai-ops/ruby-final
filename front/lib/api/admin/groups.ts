...
export type AdminListGroups = {
  groups: GroupType[];
};

export type AdminGetGroupDetails = {
  members: UserTypeWithWorkspaces[];
  group: GroupType;
};

// Resolve a group by sId for Admin, regardless of kind.
//
// `GroupResource.fetchById` deliberately does not surface the internal kinds (`regular_auto`
// groups backing a space's members/editors, `system`) — outside Admin those are
// reached from the resource they back. Admin is the exception: its group permissions tables list
...
export async function fetchAdminGroupById(
  auth: Authenticator,
  id: string
