...
import { makeGcsConsoleUrl, makeGcsUri } from "@app/lib/admin/gcs";
...
export type AdminFrameListItem = {
  sId: string;
  fileName: string;
...
export type AdminListFrames = {
  items: AdminFrameListItem[];
  totalCount: number;
};
...
export type AdminFrameStorageLocation = {
  label: string;
  gcsUri: string;
  consoleUrl: string | null;
};

export type AdminFramePublicationDatabase = {
  name: string;
  schemaSource: string;
  schemaSha256: string;
};

export type AdminFramePublication = {
  publicationId: string;
  publishedAt: string;
  publisher: string | null;
  uiBundleSha256: string;
  sourceFiles: { path: string; contentSha256: string }[];
  databases: AdminFramePublicationDatabase[];
};
 ...
 */
export type AdminFrameSharing = {
  scope: FileShareScope;
  sharedAt: number;
  shareUrl: string;
} | null;

export type AdminFrameDetails = {
  frame: AdminFrameListItem;
  ...
  publication: AdminFramePublication | null;
...
export async function getFrameDetails(
  ...
  const listItem = toAdminFrameListItem(frame, {
    functionCount: functionCounts.get(frame.id) ?? 0,
 ...
 */
export type AdminFrameFunction = {
  sId: string;
...
export type AdminFrameFunctionDetails = AdminFrameFunction & {
...
export type AdminListFrameFunctions = {
  items: AdminFrameFunction[];
};

export type AdminGetFrameFunction = {
  frameFunction: AdminFrameFunctionDetails;
};

export type AdminGetFrameFunctionSource = {
  source: string;
};
...
export async function listFrameFunctions(
  ...
  return sandboxFunctions.map((sandboxFunction) =>
    sandboxFunction.toAdminFrameJSON()
  );
}
...
// `AdminPodDatabase` in `lib/api/admin/projects.ts` for the same pattern on the pod side).
export type AdminFrameDatabase = LiveDatabaseEntry;

export type AdminListFrameDatabases = {
  items: AdminFrameDatabase[];
};
