...
/**
 * A workspace member as shown in the Admin members table. `seatType` is the
 ...
 */
export type AdminWorkspaceMember = UserTypeWithWorkspaces & {
  scheduledSeatType?: MembershipSeatType | null;
  scheduledSeatChangeAt?: number | null;
};

export type AdminGetMemberships = {
  members: AdminWorkspaceMember[];
  pendingInvitations: MembershipInvitationTypeWithLink[];
};

export type AdminSearchWorkspaceMember = {
  sId: string;
  fullName: string | null;
  email: string;
};

export type AdminSearchWorkspaceMembers = {
  members: AdminSearchWorkspaceMember[];
  total: number;
};

const adminSearchWorkspaceMemberSchema = z.object({
  sId: z.string(),
  fullName: z.string().nullable(),
  email: z.string(),
});

const adminSearchWorkspaceMembersSchema = z.object({
  members: z.array(adminSearchWorkspaceMemberSchema),
  total: z.number(),
});

export function parseAdminSearchWorkspaceMembers(
  data: unknown
): AdminSearchWorkspaceMembers {
  const result = adminSearchWorkspaceMembersSchema.safeParse(data);
  if (!result.success) {
