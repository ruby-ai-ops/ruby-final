...
// serialized for the admin UI. Historical (already-ended) phases are omitted.
export async function getSeatLimitSchedule(
  auth: Authenticator
): Promise<AdminSeatLimitScheduleResponseBody> {
  const workspace = auth.getNonNullableWorkspace();
  const schedule = await WorkspaceSeatLimitResource.fetchScheduleByWorkspace({
    workspace,
  });

  const result: AdminSeatLimitScheduleResponseBody["schedule"] = {};
