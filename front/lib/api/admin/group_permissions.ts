...
export type AdminGroupPermissionType = {
  grantType: GrantType;
...
export type AdminListGroupPermissions = {
  groupPermissions: AdminGroupPermissionType[];
};
...
async function serializeGroupPermissions(
...
): Promise<AdminGroupPermissionType[]> {
  ...
  return removeNulls(
    groupPermissions.map((gp) => {
      ...
      return group ? gp.toAdminJSON(group) : null;
    })
  );
}
...
export async function getAdminGroupPermissionsForResource(
  auth: Authenticator,
  {
    resourceType,
    resourceId,
...
export async function getAdminGroupPermissionsForGroup(
  auth: Authenticator,
  group: GroupResource
): Promise<AdminGroupPermissionType[]> {
