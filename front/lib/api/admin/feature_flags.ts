...
export interface AdminFeatureFlagUsage {
  name: string;
...
export interface AdminFeatureFlagWorkspace {
  workspaceId: string;
...
export interface GetAdminFeatureFlagsResponseBody {
  featureFlags: AdminFeatureFlagUsage[];
}

export interface GetAdminFeatureFlagWorkspacesResponseBody {
  globalRolloutPercentage: number | null;
  workspaces: AdminFeatureFlagWorkspace[];
  totalCount: number;
}
 ...
 */
export async function listFeatureFlagUsage(): Promise<AdminFeatureFlagUsage[]> {
  ...
  const configuredFlags: AdminFeatureFlagUsage[] = WHITELISTABLE_FEATURES.map(
    (name) => ({
      name,
  ...
  const legacyFlags: AdminFeatureFlagUsage[] = [...countByName.entries()]
...
export async function listWorkspacesForFeatureFlag(
  name: string
): Promise<GetAdminFeatureFlagWorkspacesResponseBody> {
