...
} from "@app/lib/api/admin/mcp_diagnostics_types";
import { MCP_DIAGNOSTIC_CHECK_NAMES } from "@app/lib/api/admin/mcp_diagnostics_types";
...
> {
  ...
  if (serverViewId) {
    // Admin admin diagnostics: surface a restricted server's view too.
...
export async function runMCPDiagnostics(
  ...
  return {
    ...
    summary,
    checks: results,
    admin_url: `${config.getAdminAppUrl()}/${workspaceId}`,
  };
}
