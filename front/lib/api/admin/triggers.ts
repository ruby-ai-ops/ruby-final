...
export const AdminTriggersSearchBodySchema = AutomationTriggersBodySchema.omit({
  format: true,
}).extend({
  sortOrder: z.enum(CONSUMPTION_TOP_SORT_ORDER).optional().default("desc"),
});

export type AdminTriggersSearchBody = z.infer<
  typeof AdminTriggersSearchBodySchema
>;
...
export async function fetchAdminTriggers(
  auth: Authenticator,
  {
    period,
    limit,
...
export type AdminGetTriggerExecutionStats = {
...
export type AdminGetTriggerDetails = {
  trigger: TriggerType;
