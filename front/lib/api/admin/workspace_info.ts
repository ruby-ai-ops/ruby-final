...
// Narrow wire shape for the Stripe subscription embedded in AdminWorkspaceInfo.
...
// response inference, so we only expose the fields the Admin admin UI actually
// reads.
export type AdminStripeSubscriptionWire = {
  id: string;
  current_period_start: number;
  current_period_end: number;
};

export type AdminCreditUsageConfig = ReturnType<
  CreditUsageConfigurationResource["toJSON"]
>;

// The four programmatic alerts (cap / 80% warning / low / critical) — id and
// current status — for deep-linking and display from Admin. Null per slot when
// that alert isn't configured.
export type AdminProgrammaticAlerts = {
...
export type AdminWorkspaceInfo = {
  ...
  // this tells Admin which of them are overridden rather than plan-given.
  ...
  // for deep-linking and display from Admin. Null when not configured / not
  // Metronome-billed.
  ...
  // The rate-limiter's verdict for the programmatic monthly cap (the admin badge):
...
export type AdminGetWorkspaceInfo = AdminWorkspaceInfo;

/**
 * Assembles the workspace-info payload shown in the admin admin UI: every
 ...
 * row cannot be found.
 */
export async function getAdminWorkspaceInfo(
  auth: Authenticator
): Promise<Result<AdminWorkspaceInfo, "workspace_not_found">> {
  ...
  // Spend-cap dimensions for the contract billing cycle, surfaced in Admin for
  ...
  // Rate-limiter verdict for the programmatic monthly cap — the admin badge
  ...
  // Resolve the Metronome alert ids backing each credit dimension so Admin can
  ...
  let programmaticAlerts: AdminProgrammaticAlerts = {
    cap: null,
    warning: null,
    low: null,
    critical: null,
  ...
  if (stripeSubscription) {
    ...
    if (lookup.isOk()) {
      ...
      logger.warn(
        ...
        "[Admin workspace-info] Failed to resolve Stripe customer ID from Metronome billing config"
      );
    }
  }
