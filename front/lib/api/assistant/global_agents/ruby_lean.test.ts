...
describe("Ruby Lean", () => {
  it("requires its feature flag for listing and direct fetches", async () => {
    ...
    await FeatureFlagFactory.basic(authenticator, "ruby_lean_agent");
  ...
  it.each([
    "light",
    "full",
  ] as const)("uses Ruby's model with no configured capabilities (%s)", async (variant) => {
    const { authenticator } = await createResourceTest({ role: "admin" });
    await FeatureFlagFactory.basic(authenticator, "ruby_lean_agent");
    const [ruby, lean] = await getGlobalAgents(
      authenticator,
      [GLOBAL_AGENTS_SID.RUBY, GLOBAL_AGENTS_SID.RUBY_LEAN],
      variant
    );
    expect(lean.model).toEqual(ruby.model);
    ...
    await upsertGlobalAgentSettings(authenticator, {
      agentId: GLOBAL_AGENTS_SID.RUBY_LEAN,
