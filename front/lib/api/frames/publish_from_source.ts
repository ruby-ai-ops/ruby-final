...
> {
  const normalizedManifestPath =
    RubyFileSystem.normalizeScopedPath(manifestPath);
  const normalizedLegacyPath =
    RubyFileSystem.normalizeScopedPath(legacyEntryPath);
  if (
  ...
  const manifestWriteAccess = rubyFs.checkWriteAccess(normalizedManifestPath);
  if (manifestWriteAccess.isErr()) {
    return new Err(manifestWriteAccess.error);
  }
  const legacyWriteAccess = rubyFs.checkWriteAccess(normalizedLegacyPath);
  ...
  const manifestMountFilePath = rubyFs.toMountFilePath(normalizedManifestPath);
  const legacyMountFilePath = rubyFs.toMountFilePath(normalizedLegacyPath);
  ...
  if (manifestFile) {
    return frameError(
      "invalid_source",
      `A separate Frame already exists at ${normalizedManifestPath}, so it cannot replace the legacy Frame. Stop and ask the user whether to delete it from the Ruby UI or keep both Frames.`
    );
  }
...
async function replaceLegacyFrameFromSource(
  ...
  // fail the publish.
  const legacyEntryDeletion = await target.rubyFs.delete(
...
async function readFrameV2SourceWithSourceLockHeld(
  ...
  return readFrameV2SourceTree(rubyFs, canonicalManifestPath);
}
