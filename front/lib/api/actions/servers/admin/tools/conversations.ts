...
// the MCP server view to the admin agent.
...
export const conversationHandlers: ConversationHandlers = {
  [GET_CONVERSATION_DETAILS_TOOL_NAME]: async (
    ...
    const conversationRes = await getAdminConversation(
    ...
    return jsonResponse({
      ...
      admin_url: `${config.getAdminAppUrl()}/${workspace_id}/conversations/${conversation_id}`,
    });
  },
  ...
  [GET_MCP_SERVER_DETAILS_TOOL_NAME]: async (
    ...
    return jsonResponse({
      ...
      admin_url: `${config.getAdminAppUrl()}/${workspace_id}`,
    });
  },
};
