...
describe("admin tools - security gates", () => {
  describe("get_workspace_metadata", () => {
    ...
    it("denies access when admin_mcp feature flag is disabled", async () => {
      const workspace = await WorkspaceFactory.basic();
      const adminAuth = await Authenticator.internalAdminForWorkspace(
        workspace.sId
      );
      ...
      // Mock getFeatureFlags to return an empty array (no admin_mcp flag).
      ...
      expect(result.isErr()).toBe(true);
      if (result.isErr()) {
        expect(result.error.message).toContain("admin_mcp feature flag");
      }
    ...
    it("returns workspace metadata when all security gates pass", async () => {
      ...
      // Mock getFeatureFlags to include admin_mcp.
      const authModule = await import("@app/lib/auth");
      vi.spyOn(authModule, "getFeatureFlags").mockResolvedValueOnce([
        "admin_mcp",
      ]);
