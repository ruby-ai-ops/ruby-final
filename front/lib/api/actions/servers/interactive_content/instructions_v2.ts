...
- Use \`@ruby-ai/slideshow/v2\` for new presentations. Keep v1 imports only when editing an existing v1 slideshow.
...
\`Document\` from \`@ruby-ai/document/v1\` provides rich text editing and autosaves to a JSON file in
...
- Imports are limited to \`react\`, \`recharts\`, \`lucide-react\`, \`papaparse\`, \`shadcn\`, \`@viz/lib/utils\`, \`@ruby-ai/frame\`, \`@ruby-ai/react-hooks\`, \`@ruby-ai/slideshow/v2\`, ${hasDocuments ? "`@ruby-ai/document/v1`, " : ""}\`motion/react\`, legacy \`@ruby-ai/slideshow/v1\` imports only when editing an existing v1 slideshow, and frame file references.
