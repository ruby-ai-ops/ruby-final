import {
  ...
  getRubyMcpServerSettingsFromMetadata,
  isRedirectUriAllowed,
  redirectUriMatchesAllowedPattern,
  validateRubyMcpServerAllowedRedirectUris,
  validateRubyMcpServerRedirectUri,
} from "@app/lib/api/mcp_server/ruby_mcp_server_settings";
import { describe, expect, it } from "vitest";

describe("ruby_mcp_server_settings", () => {
  it("returns defaults when metadata is unset", () => {
    expect(getRubyMcpServerSettingsFromMetadata(undefined)).toEqual({
      disabled: false,
      acceptAllRedirectUris: true,
      allowedRedirectUris: [...DEFAULT_RUBY_MCP_SERVER_ALLOWED_REDIRECT_URIS],
    });
  });
  ...
  it("validates redirect URIs", () => {
    ...
    expect(validateRubyMcpServerRedirectUri("not-a-uri").isErr()).toBe(true);
  });

  it("rejects duplicate redirect URIs", () => {
    const result = validateRubyMcpServerAllowedRedirectUris([
      "http://localhost:*",
      "http://localhost:*",
    ]);
