...
// The two error envelopes a rbx db command can print: the runner's typed `{ok:false}`
// (DbCommandError in cli/ruby-sandbox/functions-runner/db/common.ts) and rbx's own bare
...
// `rbxErrorCode` is the code for rbx's bare `{error}` envelope: reconcile_blocked where the
// model supplies the inputs rbx validates (reconcile takes a model-supplied schema path);
...
function dbErrorToSandboxFunctionError(
  ...
  rbxErrorCode: SandboxFunctionErrorCode
  ...
  return new SandboxFunctionError(rbxErrorCode, `${prefix}${envelope.error}`);
}
...
// Run a `rbx db` command as agent-proxied on an already-ready owner sandbox and parse its
...
async function execDbCommandOnReadySandbox<S extends z.ZodTypeAny>(
  ...
  const { rbxStdout, hashes } = stagingCapture
    ? splitStagingStdout(execResult.value.stdout)
    : { rbxStdout: execResult.value.stdout, hashes: {} };
  const envelope = parseDbEnvelope(rbxStdout, schema, what);
  if (envelope.isErr()) {
    return envelope;
  }
  return new Ok({
...
/**
 * `rbx db reconcile <name> <schema-file>`: plan the DDL for the database's drizzle schema file,
...
export async function reconcileDatabaseOnReadySandbox(
  ...
  const result = await execDbCommandOnReadySandbox(auth, sandbox, {
    ...
    command: `set -euo pipefail\n${RBX_BIN_PATH} db reconcile -- ${shellEscape(database)} ${shellEscape(schemaFileSandboxPath)}`,
    schema: reconcileEnvelopeSchema,
    what: `rbx db reconcile ${database}`,
  });
  if (result.isErr()) {
    return result;
  }
 ...
 *
 * Deliberately NOT a rbx subcommand: rbx is the agent-facing CLI, and a destructive database
...
// Success mirrors the `rbx db list` envelope in cli/ruby-sandbox/src/commands/db/list.rs.
...
/** `rbx db list` against an already-ready sandbox, whatever owns it (Pod or Frame). */
export async function listDatabasesOnReadySandbox(
  auth: Authenticator,
  sandbox: SandboxResource
): Promise<Result<LiveDatabaseEntry[], SandboxFunctionError>> {
  const result = await execDbCommandOnReadySandbox(auth, sandbox, {
    command: `${RBX_BIN_PATH} db list`,
...
// `rbx db schema` writes the regenerated file and prints only `{ok}`.
...
/**
 * `rbx db schema`: regenerate a drizzle schema file from the live database and read its text
...
export async function getDatabaseSchemaOnReadySandbox(
  ...
  const result = await execDbCommandOnReadySandbox(auth, sandbox, {
    command: [
      ...
      `${RBX_BIN_PATH} db schema -- ${shellEscape(database)} ${shellEscape(outPath)}`,
...
/**
 * `rbx db query`: execute one SQL statement (stdin) against a live database. The runner allows
 * SELECT and DML but refuses DDL/PRAGMA/ATTACH, so the schema only evolves through reconcile.
 * Results are capped by the runner (see `collectRows` in cli/ruby-sandbox/functions-runner), which
...
export async function queryDatabaseOnReadySandbox(
  ...
  const result = await execDbCommandOnReadySandbox(auth, sandbox, {
    // `--` stops the model-influenced database name from being read as a flag.
    command: `set -euo pipefail\n${RBX_BIN_PATH} db query -- ${shellEscape(database)}`,
    schema: queryEnvelopeSchema,
    what: `rbx db query ${database}`,
    stdin: sql,
  });
  if (result.isErr()) {
    return result;
...
/**
 * Parse the one-line JSON envelope a rbx command prints as its last non-empty stdout line
