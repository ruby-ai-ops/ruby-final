...
if [ ! -s "$CA_PATH" ]; then
  /usr/bin/printf '%s\n' "rbx CA file $CA_PATH missing or empty" >&2
  exit 1
fi
...
if ! /usr/bin/openssl x509 -in "$CA_PATH" -out "$normalized_ca_tmp" -outform PEM >/dev/null 2>&1; then
  /usr/bin/printf '%s\n' "rbx CA file $CA_PATH is not a valid X.509 PEM certificate" >&2
  exit 1
fi

# Snapshot the untouched system bundle once, before we ever fold in the rbx
...
# so install only the runtime rbx CA and reproduce the same bundle/hash-symlink
