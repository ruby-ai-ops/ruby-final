...
function expectedWrappedCommand(cmd: string, timeoutSec = 60): string {
  return [
    `RUBY_PROFILE=anthropic source /opt/ruby/profile/common.sh && shell "$(cat <<'RUBY_CMD_EOF'`,
    cmd,
    "RUBY_CMD_EOF",
    `)" ${timeoutSec}`,
  ].join("\n");
...
describe("wrapCommandWithCapture", () => {
  it("includes tee redirect to output file", () => {
    ...
    expect(result).toContain("tee /tmp/ruby_exec_abc123.out");
  });

  it("includes exit sentinel", () => {
    const result = wrapCommandWithCapture("ls -la", "abc123", "anthropic");
    expect(result).toContain("echo $_EXIT > /tmp/ruby_exec_abc123.exit");
  });
...
describe("buildWaitAndCollectCommand", () => {
  it("includes PID-based orphan cleanup", () => {
    ...
    expect(result).toContain("kill $(cat /tmp/ruby_wac_abc123.pid)");
  });

  it("writes its own PID", () => {
    const result = buildWaitAndCollectCommand("abc123");
    expect(result).toContain("echo $$ > /tmp/ruby_wac_abc123.pid");
  });

  it("waits for exit sentinel", () => {
    const result = buildWaitAndCollectCommand("abc123");
    expect(result).toContain(
      "while [ ! -f /tmp/ruby_exec_abc123.exit ]; do sleep 0.5; done"
    );
  });

  it("reads the output file", () => {
    const result = buildWaitAndCollectCommand("abc123");
    expect(result).toContain("cat /tmp/ruby_exec_abc123.out");
  });

  it("exits with the captured exit code", () => {
    const result = buildWaitAndCollectCommand("abc123");
    expect(result).toContain("exit $(cat /tmp/ruby_exec_abc123.exit)");
  });
});
