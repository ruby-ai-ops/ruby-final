#!/bin/bash
# Sourced by common.sh when RUBY_PROFILE=gemini.
# Dev override: RUBY_TOOLS_CMD="bun run $SCRIPT_DIR/src/index.ts"

read_file()    { run_ruby_tool --profile gemini read_file "$@"; }
write_file()   { run_ruby_tool --profile gemini write_file "$@"; }
edit_file()    { run_ruby_tool --profile gemini edit_file "$@"; }
grep_files()   { run_ruby_tool --profile gemini grep_files "$@"; }
glob()         { run_ruby_tool --profile gemini glob "$@"; }
list_dir()     { run_ruby_tool --profile gemini list_dir "$@"; }
