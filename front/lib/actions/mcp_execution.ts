 ...
 * file system by persistToolOutput (agent-loop only). The scoped path pointer is what lets the
 * model read the rest of the content back, so it must always be present. The
 * "[Full content archived at <path>]" sentence is a stable contract and must stay byte-identical.
 */
