 ...
 * revision in X-Ruby-If-Revision-Match. Read-only hosts MUST reject writes before any request.
  ...
  const readFile = useCallback(
    async (fileId: string): Promise<CommandResultMap["getFile"]> => {
      ...
      const parsedRevision = FileRevisionSchema.safeParse(
        response.headers.get(RUBY_FILE_REVISION_HEADER)
      );
      ...
      return {
        ...
        canWrite:
          ...
          revision !== null &&
          response.headers.get(RUBY_FILE_CAN_WRITE_HEADER) === "true",
      };
    },
