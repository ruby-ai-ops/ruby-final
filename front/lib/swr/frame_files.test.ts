...
describe("Frame file access", () => {
  it.each([
    ...
    clientFetch.mockResolvedValue(
      new Response("{}", {
        headers: {
          [RUBY_FILE_REVISION_HEADER]: "123",
          [RUBY_FILE_CAN_WRITE_HEADER]: "true",
        },
      })
    );
  ...
  it.each([
    ...
    clientFetch.mockResolvedValueOnce(
      new Response(null, {
        headers: { [RUBY_FILE_REVISION_HEADER]: "124", ETag: 'W/"124"' },
      })
    );
    ...
    expect(clientFetch).toHaveBeenLastCalledWith(
      "/api/w/w_test/files/path/conversation-c_test/report/notes.json",
      {
        ...
        headers: {
          "Content-Type": "application/json",
          [RUBY_IF_REVISION_MATCH_HEADER]: "123",
        },
        body: edit.content,
      }
    );
    ...
    clientFetch.mockResolvedValue(
      new Response("{}", {
        headers: {
          ETag: '"123"',
          [RUBY_FILE_CAN_WRITE_HEADER]: "true",
          ...(revision !== null && { [RUBY_FILE_REVISION_HEADER]: revision }),
        },
      })
    );
    const { result } = renderHook(() => useFrameFiles(options));
  ...
  it.each([
    ...
    clientFetch.mockResolvedValue(
      new Response(null, { headers: { [RUBY_FILE_REVISION_HEADER]: "124" } })
    );
  ...
  it.each([
    ...
    clientFetch.mockResolvedValue(
      new Response("{}", {
        headers: {
          [RUBY_FILE_REVISION_HEADER]: "123",
          [RUBY_FILE_CAN_WRITE_HEADER]: String(canWrite),
        },
      })
    );
    const { result } = renderHook(() => useFrameFiles(options));
