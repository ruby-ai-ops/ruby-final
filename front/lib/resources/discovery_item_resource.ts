...
export class DiscoveryItemResource extends BaseResource<GroupPinnedItemModel> {
  ...
  static async setPinnedForGroup(
    ...
    Result<
      ...
      RubyError<"invalid_request_error" | "group_not_found" | "unauthorized">
    >
  > {
    ...
    return withTransaction(async (t) => {
      ...
      if (!group) {
        return new Err(
          new RubyError("group_not_found", "Group not found in this workspace.")
        );
      }
  ...
  static async removePinnedForGroup(
    ...
    Result<number, RubyError<"invalid_request_error" | "unauthorized">>
  > {
...
function validatePinnedItem(
  item: PinnedDiscoveryItemInput
): Result<undefined, RubyError<"invalid_request_error">> {
  ...
  if (item.itemId.length === 0) {
    return new Err(
      new RubyError("invalid_request_error", "Discovery item ID is required.")
    );
  }
...
function invalidPositionError(): Err<RubyError<"invalid_request_error">> {
  return new Err(
    new RubyError(
      "invalid_request_error",
...
function unauthorizedPinMutation(): Err<RubyError<"unauthorized">> {
  return new Err(
    new RubyError(
      "unauthorized",
