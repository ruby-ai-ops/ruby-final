...
describe("SandboxResource.ensureActive", () => {
  ...
  describe("when the registered image has moved on", () => {
    ...
    it("leaves an outdated sleeping sandbox untouched with wakeOnly", async () => {
      const frame = await createFrameInPod();
      // The factory's image (ruby-base 0.0.0-test) is outdated against the mocked current one.
      await SandboxFactory.createForFrame(authenticator, frame, {
        status: "sleeping",
      });
