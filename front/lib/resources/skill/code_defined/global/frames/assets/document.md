...
Import `Document` from `@ruby-ai/document/v1` and render `<Document path="./content.json" />`.
...
Place `{"type":"rubyVisual","attrs":{"name":"revenue"}}` where the visual belongs in the JSON.
