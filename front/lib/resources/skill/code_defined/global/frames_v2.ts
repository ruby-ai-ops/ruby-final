...
its values for this Frame. Pass \`theme\` to the content's root: \`FrameRoot\` from \`@ruby-ai/frame\` for
pages and dashboards, or \`Slideshow\` from \`@ruby-ai/slideshow/v2\` for presentations. Both use the same
...
\`\`\`tsx
import { FrameRoot } from "@ruby-ai/frame";
...
place and publish it with \`rbx frame publish /files/<scope>/<frame>.tsx\`.
...
6. Publish the manifest, naming the legacy entry it replaces:
   ...
   \`\`\`bash
   rbx frame publish /files/<scope>/dashboards/Sales/manifest.json \\
   ...
   whether to delete it from the Ruby UI, then retry, or keep both Frames.
...
the legacy entry in place, and publish it with \`rbx frame publish /files/<scope>/<frame>.tsx\`.
