  ...
  implements WithAccessControl
{
  ...
  async updateEditorsFromDelta(
    ...
    if (updateRes.isErr()) {
      ...
      if (error instanceof RubyError && error.code === "user_not_found") {
        return new Err(
          new RubyError(
            "user_not_found",
   ...
   * and callers MUST NOT add a space-read check on top. Editors and workspace admins hold it; a Admin
