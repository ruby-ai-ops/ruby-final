...
function changeForSuggestion(
...
): Result<AgentChange, RubyError<"invalid_request_error">> {
  ...
  if (!parsed.success) {
    return new Err(
      new RubyError("invalid_request_error", "Unsupported agent suggestion.")
    );
  }
...
): Result<AgentBatchChanges, RubyError<"invalid_request_error">> {
