...
describe("Authenticator.fromJSON", () => {
  ...
  it("keeps a user-less admin super-user's workspace-wide groups across refresh", async () => {
    const workspace = await WorkspaceFactory.basic();
    const adminAuth = await Authenticator.internalAdminForWorkspace(
      workspace.sId
    );
    ...
    // Cloudflare admin operator with no Ruby user: reads every space via all workspace groups.
    const auth = await Authenticator.fromRubySuperUser({
      wId: workspace.sId,
      adminPrincipal: { email: "admin@ruby.ad", name: "Admin" },
    });
