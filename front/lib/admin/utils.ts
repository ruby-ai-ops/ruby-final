...
import type {
  AdminDataSourceType,
  AdminDataSourceViewType,
  AdminItemBase,
  AdminMCPServerViewType,
  AdminSpaceType,
} from "@app/types/admin";

export async function spaceToAdminJSON(
  auth: Authenticator,
  space: SpaceResource
): Promise<AdminSpaceType> {
...
export async function dataSourceToAdminItem(
  ...
  return {
    id: dataSource.id,
    link: workspace
      ? `${config.getAdminAppUrl()}/${workspace.sId}/data_sources/${dataSource.sId}`
      : null,
    name:
...
async function dataSourceToAdminJSON(
  auth: Authenticator,
  dataSource: DataSourceResource
): Promise<AdminDataSourceType> {
  return {
    ...dataSource.toJSON(),
    ...(await dataSourceToAdminItem(dataSource)),
    space: await spaceToAdminJSON(auth, dataSource.space),
  };
}

export async function dataSourceViewToAdminItem(
  ...
  return {
    ...
    link: workspace
      ? `${config.getAdminAppUrl()}/${workspace.sId}/spaces/${dataSourceView.space.sId}/data_source_views/${dataSourceView.sId}`
      : null,
    name:
      (workspace ? `${workspace.name}'s ` : "") +
...
export async function dataSourceViewToAdminJSON(
  auth: Authenticator,
  dataSourceView: DataSourceViewResource
): Promise<AdminDataSourceViewType> {
  return {
    ...dataSourceView.toJSON(),
    dataSource: await dataSourceToAdminJSON(auth, dataSourceView.dataSource),
    ...(await dataSourceViewToAdminItem(dataSourceView)),
    space: await spaceToAdminJSON(auth, dataSourceView.space),
  };
}

export async function mcpServerViewToAdminJSON(
  mcpServerView: MCPServerViewResource,
  auth: Authenticator
): Promise<AdminMCPServerViewType> {
  const workspace = await WorkspaceResource.fetchByModelId(
  ...
  return {
    ...
    link: workspace
      ? `${config.getAdminAppUrl()}/${workspace.sId}/spaces/${mcpServerView.space.sId}/mcp_server_views/${mcpServerView.sId}`
      : null,
    name: json.name ?? json.server.name,
    customName: json.name,
    type: "MCP Server View",
    space: await spaceToAdminJSON(auth, mcpServerView.space),
