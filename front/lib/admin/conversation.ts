...
export async function getAdminConversation(
...
): Promise<Result<AdminConversationType, ConversationError>> {
  ...
  if (conversation.isOk()) {
    const adminConversation = conversation.value as AdminConversationType;

    const agentMessages = adminConversation.content
      .flat()
      .filter((m): m is AdminAgentMessageType => m.type === "agent_message");
    ...
    for (const messages of adminConversation.content) {
        ...
        if (m.type === "agent_message") {
          ...
          if (runIds) {
            m.runUrls = runIds.map((runId) => {
              ...
              const url = `/admin/${owner.sId}/llm-traces/${runId}`;
    ...
    return new Ok(adminConversation);
  }
