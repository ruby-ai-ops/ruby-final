...
export const AnthropicClaudeOpusFourDotSixGlobalAnthropicStreamSetup: StreamSetup =
  {
    createInstance: () =>
    ...
    //
    //   - `t-0` / `t-0.1` at any effort but "none". Unlike Opus 4.7+, 4.6 still
    ...
    tests: {
      ...
      // Thinking hardly ever occurs at low reasoning effort: with adaptive
      // thinking the model decides whether to think at all, and low effort skips
      // it entirely on easy inputs. Opus 4.6 skips it on this prompt — 0 thinking
      // tokens on 15/15 replays of the exact payload we send, on both endpoints,
      // while `medium` thinks every time and Sonnet 4.6 thinks even at `low`.
      // `HAS_REASONING` therefore asserts a model decision rather than an API
      // contract, and would be flaky whichever way it went; assert only that the
      // call succeeds.
...
// NODE_ENV=test RUN_LLM_TEST=true npm run test -- --config lib/model_constructors/test/vite.config.js --bail 1 lib/model_constructors/test/endpoints/anthropic_claude_opus_four_dot_six_global_anthropic.test.ts
