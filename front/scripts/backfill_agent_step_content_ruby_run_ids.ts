/**
 * Backfill the rubyRunId stamped on model-produced agent step contents. This must run before the
 ...
 *
 * The script considers every agent-message status and only updates null rubyRunId values, making
 * reruns safe. An in-progress message may be skipped until its newest run id has been persisted;
 * rerunning after it settles will pick it up. Cache reads compare rubyRunId with Postgres metadata,
 ...
 * applies the date and rubyRunId filters in memory. This avoids requiring a one-off index and keeps
 * each database query bounded even when most of a workspace's contents predate the requested range.
 *
 * Run once in each region.
 *
 * Dry run:
 *   npx tsx scripts/backfill_agent_step_content_ruby_run_ids.ts \
 ...
 * invariant.
 */
export function inferRubyRunIdForStepContent(
  ...
  return run.rubyRunId;
}

export function getChronologicalRunsForAgentMessage(
  rubyRunIds: string[],
  runByRubyRunId: Map<string, RunCandidate>
): RunCandidate[] | null {
  const uniqueRubyRunIds = [...new Set(rubyRunIds)];
  const messageRuns = uniqueRubyRunIds.flatMap((rubyRunId) => {
    const run = runByRubyRunId.get(rubyRunId);
    return run ? [run] : [];
  });

  // A missing run shifts every later positional match, so do not infer any row for the message.
  if (messageRuns.length !== uniqueRubyRunIds.length) {
    return null;
  }
...
export async function listStepContentBatch({
  ...
  // Do not put the date or rubyRunId predicates in this query. Neither is covered by the existing
...
async function inferStepContentUpdates({
  ...
  const runs = await RunModel.findAll({
    attributes: ["id", "rubyRunId", "createdAt"],
    where: {
      workspaceId: workspace.id,
      rubyRunId: { [Op.in]: rubyRunIds },
    },
  });
  const runByRubyRunId = new Map(
    runs.map((run) => [
      run.rubyRunId,
      {
  ...
  return candidates.flatMap((candidate) => {
    ...
    const rubyRunId = inferRubyRunIdForStepContent(
      candidate,
...
async function applyStepContentUpdates({
  ...
  const values = updates.map((update, index) => {
    ...
    bind[`rubyRunId${index}`] = update.rubyRunId;
    return `($stepContentModelId${index}::bigint, $rubyRunId${index}::text)`;
  });
  ...
  makeScript(
    ...
    async (
        ...
        async (workspace) => {
          ...
          while (true) {
            ...
            logger.info(
              ...
              "[StepContentRubyRunIdBackfill] Batch complete"
            );
          }
          ...
          logger.info(
            ...
            "[StepContentRubyRunIdBackfill] Workspace complete"
          );
        },
      ...
      logger.info(
        ...
        "[StepContentRubyRunIdBackfill] Backfill complete"
      );
    }
  );
}

if (process.argv[1]?.endsWith("backfill_agent_step_content_ruby_run_ids.ts")) {
  runScript();
}
