...
describe("InputBarAttachments", () => {
  ...
  it("renders every attachment as a card while an image is uploading", () => {
    const { container } = renderAttachments({
      fileBlobs: [
        makeFileBlob({
          filename: "photo.png",
    ...
    // The uploading image shows a spinner and the PDF is a card, not a chip.
  ...
  it("renders chips side by side when no image is attached", () => {
    const { container } = renderAttachments({
      fileBlobs: [
