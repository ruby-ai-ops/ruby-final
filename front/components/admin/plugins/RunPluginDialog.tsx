import { PluginForm } from "@app/components/admin/plugins/PluginForm";
import {
  AdminAlert,
  AdminAlertDescription,
  AdminAlertTitle,
} from "@app/components/admin/shadcn/ui/alert";
import type { PluginListItem, PluginResponse } from "@app/lib/api/admin/types";
import { getCellDisplay } from "@app/lib/admin/cells";
import {
  useAdminPluginAsyncArgs,
  useAdminPluginManifest,
  useRunAdminPlugin,
} from "@app/admin-app/swr/plugins";
    ...
    <div className="mb-4 mt-4 flex flex-col gap-2">
      {succeeded.length > 0 && (
        <AdminAlert variant="success">
          <AdminAlertTitle>Succeeded</AdminAlertTitle>
          <AdminAlertDescription>
            {succeeded
              .map(
      ...
      {failed.length > 0 && (
        <AdminAlert variant="destructive">
          <AdminAlertTitle>Failed</AdminAlertTitle>
          <AdminAlertDescription>
            {failed
              .map(
                (result) => `${getCellDisplay(result.cell)}: ${result.message}`
...
export function RunPluginDialog({
  ...
  const { isLoading, manifest } = useAdminPluginManifest({
    disabled: false,
    pluginId: plugin.id,
  });
  ...
  const { asyncArgs, isLoading: isLoadingAsyncArgs } = useAdminPluginAsyncArgs({
    disabled: !manifest || !hasAsyncArgs,
    pluginId: plugin.id,
    pluginResourceTarget,
  });

  const { doRunPlugin, doRunPluginOnCells } = useRunAdminPlugin({
    pluginId: plugin.id,
    pluginResourceTarget,
  });
          ...
          {isLoading || (hasAsyncArgs && isLoadingAsyncArgs) ? (
            ...
            <>
              {isRunning && (
                <AdminAlert>
                  <div className="flex items-center gap-3">
                    <Spinner size="sm" />
                    <div>
                      <AdminAlertTitle>Running…</AdminAlertTitle>
                      <AdminAlertDescription>
                        Still working
                        {elapsedSeconds >= 5
              ...
              {result && result.display === "text" && (
                <AdminAlert variant="success">
                  <AdminAlertTitle>Success</AdminAlertTitle>
                  <AdminAlertDescription>
              ...
              {manifest.warning && (
                <AdminAlert variant="destructive">
                  <AdminAlertTitle>Warning</AdminAlertTitle>
                  <AdminAlertDescription>
                    {manifest.warning}
                  </AdminAlertDescription>
                </AdminAlert>
