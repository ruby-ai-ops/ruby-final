...
interface AdminChangeSeatModalProps {
  isOpen: boolean;
  onClose: () => void;
  member: MemberUsageType | null;
  // Live seat-plan catalog for the workspace (see useAdminSeatPlan) — the
...
// Admin has no working write route for member seat changes yet, so Validate
// is wired to nothing but a notice rather than an actual mutation.
function useUnavailableSeatChangeSave() {
  const sendNotification = useSendNotification();
  return () =>
    sendNotification({
      title: "Not available from Admin yet",
      description: "Changing a member's seat from Admin isn't supported yet.",
      type: "info",
    });
}
...
export function AdminChangeSeatModal({
  isOpen,
  onClose,
  member,
  seatPlans,
