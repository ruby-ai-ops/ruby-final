...
import {
  useCreateAdminSkillSuggestion,
  useAdminSkills,
} from "@app/admin-app/swr/skills";
...
export function CreateSkillSuggestionSheet({
  ...
  const { data: skills, isLoading: isSkillsLoading } = useAdminSkills({
    owner,
    disabled: !show,
  });
  ...
  const { createSkillSuggestion } = useCreateAdminSkillSuggestion({
    owner,
  ...
  const handleInsertCapability = (item: SlashCommand) => {
    ...
    if (isToolSlashCommand<AdminMCPServerViewListItemType>(item)) {
      editor
        .chain()
        .focus()
        .insertToolNode({
