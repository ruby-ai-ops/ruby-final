import {
  AdminCommandGroup,
  AdminCommandItem,
} from "@app/components/admin/shadcn/ui/command";
import { useAppRouter } from "@app/lib/platform";
import { useCurrentPage } from "@app/admin-app/swr/currentPage";
import type { AdminFavorite, AdminFavoriteType } from "@app/admin-app/swr/favorites";
import { ADMIN_FAVORITE_TYPES, useAdminFavorites } from "@app/admin-app/swr/favorites";
import {
  Chip,
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
  Icon,
  IconButton,
  LinkWrapper,
  Star01,
  StarFilled,
  XClose,
} from "@ruby-ai/ui";
import type { ComponentProps } from "react";
import { useCallback, useEffect, useState } from "react";

function getFavoriteChipColor(
  type: AdminFavoriteType
): ComponentProps<typeof Chip>["color"] {
  switch (type) {
    case "Workspace":
      return "highlight";
    case "Data Source":
      return "info";
    case "Data Source View":
      return "warning";
    case "Agent":
      return "success";
    case "Frame":
      return "highlight";
    default:
      return "primary";
  }
}

function groupByType(
  items: AdminFavorite[]
): { type: AdminFavoriteType; items: AdminFavorite[] }[] {
  const byType = new Map<AdminFavoriteType, AdminFavorite[]>();
  for (const item of items) {
    const existing = byType.get(item.data.type) ?? [];
    byType.set(item.data.type, [...existing, item]);
  }
  const rank = (type: AdminFavoriteType) => {
    const index = ADMIN_FAVORITE_TYPES.indexOf(type);
    return index === -1 ? ADMIN_FAVORITE_TYPES.length : index;
  };
  return [...byType.keys()]
    .sort((a, b) => rank(a) - rank(b))
    .map((type) => ({ type, items: byType.get(type) ?? [] }));
}

export function AdminFavoriteButton() {
  const router = useAppRouter();
  const current = useCurrentPage();
  const { isFavorite, toggleFavorite } = useAdminFavorites();
  const [hasMounted, setHasMounted] = useState(false);

  const currentForPath = current?.url === router.asPath ? current : null;
  const isCurrentlyFavorite =
    currentForPath !== null && isFavorite(currentForPath.url);

  const handleToggle = useCallback(() => {
    if (currentForPath) {
      toggleFavorite(currentForPath);
    }
  }, [currentForPath, toggleFavorite]);

  useEffect(() => {
    setHasMounted(true);
  }, []);

  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "d" && (e.metaKey || e.ctrlKey)) {
        e.preventDefault();
        handleToggle();
      }
    };
    document.addEventListener("keydown", handleKeyDown);
    return () => document.removeEventListener("keydown", handleKeyDown);
  }, [handleToggle]);

  if (!hasMounted) {
    return null;
  }

  return (
    <IconButton
      icon={isCurrentlyFavorite ? StarFilled : Star01}
      onClick={handleToggle}
      disabled={currentForPath === null}
      variant="outline"
      size="sm"
      tooltip={
        currentForPath === null
          ? "Loading…"
          : isCurrentlyFavorite
            ? "Remove from favorites (⌘D)"
            : "Add to favorites (⌘D)"
      }
    />
  );
}

interface AdminNavItemRowProps {
  item: AdminFavorite;
  onRemove?: (url: string) => void;
}

function AdminNavItemRow({ item, onRemove }: AdminNavItemRowProps) {
  return (
    <div className="group flex items-center gap-3 rounded-md px-2 py-1.5 hover:bg-primary-100">
      <Chip size="xs" color={getFavoriteChipColor(item.data.type)}>
        {item.data.type}
      </Chip>
      <LinkWrapper href={item.url} className="flex min-w-0 flex-1 flex-col">
        <span className="truncate text-sm font-medium text-foreground">
          {item.data.name}
        </span>
        {item.data.subtitle && (
          <span className="truncate text-xs text-muted-foreground">
            {item.data.subtitle}
          </span>
        )}
      </LinkWrapper>
      {item.data.sId && (
        <span className="shrink-0 font-mono text-xs text-muted-foreground">
          {item.data.sId}
        </span>
      )}
      {onRemove && (
        <div className="shrink-0 opacity-0 transition-opacity group-hover:opacity-100">
          <IconButton
            icon={XClose}
            onClick={() => onRemove(item.url)}
            size="xs"
            variant="outline"
            tooltip="Remove from favorites"
          />
        </div>
      )}
    </div>
  );
}

interface AdminNavItemGroupsProps {
  items: AdminFavorite[];
  onRemove?: (url: string) => void;
}

function AdminNavItemGroups({ items, onRemove }: AdminNavItemGroupsProps) {
  return (
    <div className="flex flex-col gap-4">
      {groupByType(items).map((group) => (
        <div key={group.type}>
          <h3 className="mb-1 px-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            {group.type}
          </h3>
          <div className="flex flex-col">
            {group.items.map((item) => (
              <AdminNavItemRow key={item.url} item={item} onRemove={onRemove} />
            ))}
          </div>
        </div>
      ))}
    </div>
  );
}

export function AdminFavoritesList() {
  const [hasMounted, setHasMounted] = useState(false);
  const { favorites, removeFavorite } = useAdminFavorites();

  useEffect(() => {
    setHasMounted(true);
  }, []);

  if (!hasMounted) {
    return null;
  }

  return (
    <div className="mb-6 rounded-xl border bg-background p-4">
      <Collapsible defaultOpen={false}>
        <CollapsibleTrigger>
          <div className="flex items-center gap-2">
            <Icon visual={Star01} size="sm" className="text-muted-foreground" />
            <h2 className="text-lg font-semibold">Favorites</h2>
            {favorites.length > 0 && (
              <span className="text-sm text-muted-foreground">
                {favorites.length}
              </span>
            )}
          </div>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="mt-3">
            {favorites.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                No favorites yet. Press{" "}
                <kbd className="rounded border px-1.5 py-0.5 font-mono text-xs">
                  ⌘D
                </kbd>{" "}
                on any page (or use the star in the top bar) to add it here.
              </p>
            ) : (
              <AdminNavItemGroups items={favorites} onRemove={removeFavorite} />
            )}
          </div>
        </CollapsibleContent>
      </Collapsible>
    </div>
  );
}

interface AdminNavCommandItemProps {
  item: AdminFavorite;
  index: number;
  onNavigate: () => void;
}

function AdminNavCommandItem({
  item,
  index,
  onNavigate,
}: AdminNavCommandItemProps) {
  return (
    <AdminCommandItem
      value={`${item.data.type} ${item.data.name} ${item.data.subtitle ?? ""} ${
        item.data.sId ?? ""
      } ${item.url}`}
      index={index}
      href={item.url}
      onClick={onNavigate}
    >
      <div className="flex w-full items-center gap-3 px-2 text-foreground">
        <Chip size="xs" color={getFavoriteChipColor(item.data.type)}>
          {item.data.type}
        </Chip>
        <span className="text-sm font-medium">{item.data.name}</span>
        {item.data.subtitle && (
          <span className="truncate text-xs text-muted-foreground">
            {item.data.subtitle}
          </span>
        )}
        {item.data.sId && (
          <span className="font-mono text-xs text-muted-foreground">
            {item.data.sId}
          </span>
        )}
      </div>
    </AdminCommandItem>
  );
}

interface AdminFavoritesCommandGroupsProps {
  onNavigate: () => void;
}

export function AdminFavoritesCommandGroups({
  onNavigate,
}: AdminFavoritesCommandGroupsProps) {
  const { favorites } = useAdminFavorites();

  if (favorites.length === 0) {
    return null;
  }

  return (
    <AdminCommandGroup heading="Favorites">
      {favorites.map((item, index) => (
        <AdminNavCommandItem
          key={item.url}
          item={item}
          index={index}
          onNavigate={onNavigate}
        />
      ))}
    </AdminCommandGroup>
  );
}
