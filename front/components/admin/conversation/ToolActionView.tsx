...
import { formatDurationMs } from "@app/components/admin/conversation/MessageMetadata";
...
function getActionLabel(action: AdminAgentMessageType["actions"][number]) {
  return action.displayLabels?.done ?? action.functionCallName;
}

function getActionStatus(
  status: AdminAgentMessageType["actions"][number]["status"]
): null | {
  color: ChipColor;
  label: string;
} {
...
interface ToolActionContentProps {
  action: AdminAgentMessageType["actions"][number];
  isExpanded: boolean;
}
