import { InvocationMCPActions } from "@app/components/admin/frames/functions/mcp_actions";
import { AdminJsonBlock } from "@app/components/admin/sandbox_functions/json_block";
import type { AdminSandboxFunctionInvocation } from "@app/lib/api/admin/sandbox_functions";
import { formatCalendarDateTime } from "@app/lib/utils/timestamps";
import {
  useAdminSandboxFunctionInvocation,
  useAdminSandboxFunctionInvocations,
} from "@app/admin-app/swr/frame_function_details";
import type {
...
function elapsedLabel(
  invocation: AdminSandboxFunctionInvocation
): string | null {
...
export function FrameFunctionInvocations({
  ...
  const { invocations, isLoading, isError } = useAdminSandboxFunctionInvocations(
    {
      owner,
      frameId,
      functionId,
      ...
      <CollapsibleContent>
        {isLoading ? (
          ...
          <div className="flex flex-col gap-2 pb-2 pl-4">
            <AdminJsonBlock defaultOpen label="Input" value={details.input} />
            <AdminJsonBlock defaultOpen label="Result" value={details.result} />
            {details.error && (
              <AdminJsonBlock defaultOpen label="Error" value={details.error} />
            )}
