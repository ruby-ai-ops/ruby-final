...
import { useAdminSandboxFunctionMCPActionOutput } from "@app/admin-app/swr/frame_function_details";
...
interface InvocationMCPActionsProps {
  actions: AdminSandboxFunctionMCPAction[];
  functionId: string;
  invocationId: string;
...
interface MCPActionRowProps {
  action: AdminSandboxFunctionMCPAction;
  functionId: string;
  invocationId: string;
  owner: LightWorkspaceType;
  frameId: string;
...
function MCPActionRow({
  ...
  const { output, isLoading, isError } = useAdminSandboxFunctionMCPActionOutput({
    owner,
    frameId,
    functionId,
    invocationId,
    ...
    <Collapsible defaultOpen={false} onOpenChange={setIsOpen}>
      ...
      <CollapsibleContent>
        <div className="flex flex-col gap-2 pb-2 pl-4">
          ...
          <AdminJsonBlock defaultOpen label="Inputs" value={action.inputs} />
          {!action.hasOutput ? (
            <p className="py-1 text-sm text-muted-foreground">
              No output recorded.
            </p>
            ...
            <AdminJsonBlock defaultOpen label="Output" value={output} />
          )}
        </div>
      </CollapsibleContent>
    </Collapsible>
