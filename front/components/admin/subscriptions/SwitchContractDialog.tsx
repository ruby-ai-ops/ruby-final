import { AdminForm } from "@app/components/admin/shadcn/ui/form";
import {
  InputField,
  SelectField,
} from "@app/components/admin/shadcn/ui/form/fields";
...
} from "@app/lib/admin/switch_contract_templates";
import { SWITCH_CONTRACT_TEMPLATES } from "@app/lib/admin/switch_contract_templates";
import {
  useAdminMetronomePackages,
  useAdminPlans,
  useAdminStripeCustomerCurrency,
} from "@app/lib/swr/admin";
import { useAdminPluginAsyncArgs } from "@app/admin-app/swr/plugins";
...
import { CreditUsageConfigurationSchema } from "@app/types/admin/credit_usage_configuration";
import { SwitchContractBodySchema } from "@app/types/admin/switch_contract";
...
export default function SwitchContractDialog({
  ...
  const {
    ...
    isPackagesLoading,
    packagesError,
  } = useAdminMetronomePackages({ disabled: !open });
  ...
  const { asyncArgs: existingCreditConfig } = useAdminPluginAsyncArgs({
      ...
      const submit = async () => {
        ...
        try {
          const r = await clientFetch(
            `/api/admin/workspaces/${owner.sId}/switch_contract`,
            {
              method: "POST",
