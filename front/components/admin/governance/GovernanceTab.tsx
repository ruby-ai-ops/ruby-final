...
import type { AdminMessagingApp } from "@app/lib/api/admin/messaging_apps";
import {
  areAuditLogsEnabled,
...
import { useAdminGovernancePermissions } from "@app/admin-app/swr/governance_permissions";
import { useAdminGroups } from "@app/admin-app/swr/groups";
import { useAdminMessagingApps } from "@app/admin-app/swr/messaging_apps";
...
import { Chip, LinkWrapper, SliderToggle, Spinner } from "@ruby-ai/ui";
  ...
  return [
    {
      ...
      value: workspaceDefaultAgentId ? (
        <LinkWrapper
          href={`/admin/${owner.sId}/assistants/${workspaceDefaultAgentId}`}
        >
...
function buildMessagingAppRows(
  ...
  messagingApps: AdminMessagingApp[]
): GovernanceRow[] {
      ...
      value: (
        <div className="flex items-center gap-2">
          ...
          {messagingApp.dataSourceId && (
            <LinkWrapper
              href={`/admin/${owner.sId}/data_sources/${messagingApp.dataSourceId}`}
            >
...
export function GovernanceTab({
  ...
  } = useAdminGovernancePermissions({ owner });
  const {
    data: groups,
    isLoading: isGroupsLoading,
    isError: isGroupsError,
  } = useAdminGroups({ owner });
  const {
    data: messagingApps,
    isLoading: isMessagingAppsLoading,
    isError: isMessagingAppsError,
  } = useAdminMessagingApps({ owner });
  ...
  return (
    <div className="flex flex-grow flex-col rounded-lg border p-4">
      ...
      <AdminTable className="table-fixed">
        <AdminTableHeader>
          <AdminTableRow>
            <AdminTableHead className="w-2/5">Setting</AdminTableHead>
            <AdminTableHead className="w-1/5">Value</AdminTableHead>
            {section.showGroups && (
              <AdminTableHead className="w-1/5">Groups</AdminTableHead>
            )}
            <AdminTableHead className="w-1/5">Audit logs</AdminTableHead>
          </AdminTableRow>
        </AdminTableHeader>
        <AdminTableBody>
          {section.rows.map((row) => (
            <AdminTableRow key={row.key}>
              ...
              <AdminTableCell>{row.value}</AdminTableCell>
              {section.showGroups && (
                <AdminTableCell>
                  {row.groupIds.length === 0 ? (
                    ...
                    <div className="flex flex-wrap gap-1">
                      {row.groupIds.map((groupId) => (
                        <LinkWrapper
                          key={groupId}
                          href={`/admin/${owner.sId}/groups/${groupId}`}
                        >
                          <Chip size="xs">
