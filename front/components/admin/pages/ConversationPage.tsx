import { AgentMessageView } from "@app/components/admin/conversation/AgentMessageView";
import { ConversationActions } from "@app/components/admin/conversation/ConversationActions";
import { ConversationMessagesHeader } from "@app/components/admin/conversation/ConversationMessagesHeader";
import { AdminConversationConsumptionInspector } from "@app/components/admin/conversation/consumption_inspectors";
import {
  CompactionMessageView,
  ContentFragmentView,
  UserMessageView,
} from "@app/components/admin/conversation/MessageViews";
import { useConversationInspectorPanels } from "@app/components/admin/conversation/use_conversation_inspector_panels";
import { AdminConversationWakeUpsInspector } from "@app/components/admin/conversation/wakeups_inspector";
import { PluginList } from "@app/components/admin/plugins/PluginList";
import { useWorkspace } from "@app/lib/auth/AuthContext";
import { useRequiredPathParam } from "@app/lib/platform";
import { useAdminConversation } from "@app/admin-app/swr";
import { useAdminConversationConfig } from "@app/admin-app/swr/conversation_config";
...
export function ConversationPage() {
  ...
  } = useAdminConversationConfig({
    owner,
    conversationId,
    disabled: false,
  });

  const { conversation } = useAdminConversation({
  ...
  const { data: spaceDetails } = useAdminSpaceDetails({
    owner,
    ...
    conversation && (
      ...
      >
        <h3 className="text-xl font-bold 2xl:col-start-2">
          ...
          <LinkWrapper href={`/admin/${owner.sId}`} className="text-highlight">
            {owner.name}
          </LinkWrapper>
          {pod && (
            <>
              ...
              in pod{" "}
              <LinkWrapper
                href={`/admin/${owner.sId}/spaces/${pod.sId}`}
        ...
        <div className="flex min-w-0 flex-col gap-3 2xl:col-start-2 2xl:row-start-2">
          ...
          <div
            className={cn(
              ...
              "[--admin-inspector-width:28rem]",
              "xl:grid-cols-[minmax(0,1fr)_var(--admin-inspector-width)]"
            )}
          >
            <aside
            ...
            >
              ...
              >
                <AdminConversationConsumptionInspector
                  conversationId={conversationId}
                  isOpen={isConversationOpen}
                  onOpenChange={setConversationOpen}
                  workspaceId={owner.sId}
                />
                <AdminConversationWakeUpsInspector
                  conversationId={conversationId}
