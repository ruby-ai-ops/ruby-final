...
import {
  useAdminCacheCatalog,
  useAdminCacheDeleteAll,
  useAdminCacheInvalidate,
  useAdminCacheLookup,
} from "@app/admin-app/swr/cache";
import { useAdminPageMetadata } from "@app/admin-app/swr/currentPage";
import type {
  AdminCacheResourceDescriptor,
  RedisCacheResult,
  RedisInstance,
...
function ResourceLookupForm({ resources, onSubmit }: ResourceLookupFormProps) {
  const [selectedResource, setSelectedResource] =
    useState<AdminCacheResourceDescriptor | null>(null);
  ...
  function handleSelectResource(resource: AdminCacheResourceDescriptor) {
...
function ResourceInvalidateForm({
  ...
  const { doDeleteAll, isDeletingAll } = useAdminCacheDeleteAll();
...
export function CacheLookupPage() {
  useAdminPageMetadata({ name: "Cache Lookup" });
  const { resources } = useAdminCacheCatalog();
  ...
  const { doInvalidate, isInvalidating } = useAdminCacheInvalidate();
