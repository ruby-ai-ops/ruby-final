import { AdminChangeSeatModal } from "@app/components/admin/credits/AdminChangeSeatModal";
...
import {
  useAdminAwuPoolCurrentCycle,
  useAdminAwuPoolCycleHistory,
  useAdminMembersUsage,
  useAdminSeatPlan,
} from "@app/admin-app/swr/credits";
import { useAdminPageMetadata } from "@app/admin-app/swr/currentPage";
import { useAdminGroups } from "@app/admin-app/swr/groups";
import { useAdminAllowedModelTiers } from "@app/admin-app/swr/model_tiers";
...
// base tier), ordered lowest to highest. Admin has no per-workspace seat-plan
...
function PoolCreditCard({ owner }: PoolCreditCardProps) {
  ...
  } = useAdminAwuPoolCycleHistory({ owner, cycleHistoryLimit });
...
export function PoolUsagePage() {
  ...
  useAdminPageMetadata({ name: owner.name, subtitle: "Credits Usage" });
  ...
  const { awuPoolCurrentCycle } = useAdminAwuPoolCurrentCycle({
    owner,
  ...
  const { seatPlans, isSeatPlanLoading, isSeatPlanError } = useAdminSeatPlan({
    owner,
  });
  ...
  const { data: allGroups } = useAdminGroups({ owner });
  ...
  } = useAdminAllowedModelTiers({ owner });
        ...
        title={
          <div className="flex w-full items-center justify-between gap-4">
            ...
            <Button
              ...
              href={`/admin/${owner.sId}/analytics`}
            />
          </div>
        }
        description={
          <>
            ...
            . Admin uses workspace-admin visibility, so resolved labels may
