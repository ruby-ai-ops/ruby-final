import { SelfImprovingSkillsConversationDataTable } from "@app/components/admin/conversation/self_improving_skills_table";
...
import { SkillSuggestionDataTable } from "@app/components/admin/skill_suggestions/table";
import { SkillOverviewTable } from "@app/components/admin/skills/SkillOverviewTable";
...
import {
  useAdminSkillDetails,
  useAdminSkillVersions,
} from "@app/admin-app/swr/skill_details";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
export function SkillDetailsPage() {
  ...
  const { versions, isLoading: isLoadingVersions } = useAdminSkillVersions({
    owner,
    skillId: sId,
    disabled: false,
  });
                    ...
                    <div className="space-y-1">
                      {usedBySkills.map((usedBySkill) => (
                        <div key={usedBySkill.sId}>
                          <LinkWrapper
                            href={`/admin/${owner.sId}/skills/${usedBySkill.sId}`}
                            className="text-highlight-500"
                          >
                            {usedBySkill.name}
                          </LinkWrapper>
