...
import type { AdminFeatureFlagWorkspaceWithCell } from "@app/hooks/useAdminFeatureFlagWorkspaces";
import { useAdminFeatureFlagWorkspacesAllCells } from "@app/hooks/useAdminFeatureFlagWorkspaces";
...
function WorkspaceTogglePluginDialog({
  ...
  const { plugins, isLoading } = useAdminListPluginForResourceType({
...
function makeColumns({
  onToggleFlags,
  onSwitchCell,
}: MakeColumnsParams): ColumnDef<AdminFeatureFlagWorkspaceWithCell>[] {
  return [
    {
      id: "cell",
      accessorKey: "cell",
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Cell" />
      ),
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Workspace" />
      ),
      cell: ({ row }) => (
        <div onClick={() => onSwitchCell(row.original.cell)}>
          <LinkWrapper href={`/admin/${row.original.workspaceId}`}>
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Workspace ID" />
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Plan" />
      ),
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Enabled at" />
      ),
      cell: ({ row }) => (
...
export function FeatureFlagDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-6">
        <LinkWrapper href="/admin/feature-flags">
