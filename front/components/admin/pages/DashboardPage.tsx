import { AdminFavoritesList } from "@app/components/admin/AdminFavorites";
import {
  AdminTable,
  AdminTableBody,
  AdminTableCell,
...
import type { AdminWorkspaceWithCell } from "@app/admin-app/swr/search";
import { useAdminWorkspacesAllCells } from "@app/admin-app/swr/search";
...
const PLAN_TYPE_FILTER_LABELS: Record<AdminPlanTypeFilter, string> = {
...
interface WorkspaceListProps {
  workspaces: AdminWorkspaceWithCell[];
  isWorkspacesLoading?: boolean;
  showCell?: boolean;
  onWorkspaceClick?: (ws: AdminWorkspaceWithCell) => void;
}
          ...
          <LinkWrapper href={`/admin/${ws.sId}`}>
            <li className="border-material-100 w-80 rounded-lg border p-4 transition-colors duration-200 hover:bg-primary-100">
              ...
              <AdminTable>
                <AdminTableBody>
                  <AdminTableRow>
                    <AdminTableCell className="space-x-2" colSpan={3}>
                      <label>
...
function DashboardPageSPA() {
  useAdminPageMetadata({ name: "Home" });

  const { cellInfo, setCellInfo, cells } = useCellContext();

  const [planTypeFilter, setPlanTypeFilter] = useState<
    AdminPlanTypeFilter | undefined
  >(undefined);
  ...
  const handlePlanTypeFilterChange = useCallback(
    (filter: AdminPlanTypeFilter | undefined) => {
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        />
        {ADMIN_PLAN_TYPE_FILTERS.map((filter) => (
          <Chip
            key={filter}
            size="xs"
