import { CreateCouponForm } from "@app/components/admin/coupons/CreateCouponForm";
import {
  useAdminArchiveCoupon,
  useAdminCouponRedemptions,
  useAdminCoupons,
...
const redemptionColumns: ColumnDef<CouponRedemptionRowData>[] = [
  {
    ...
    cell: ({ row }) => (
      <DataTable.CellContent>
        <LinkWrapper
          href={`/admin/${row.original.workspaceId}`}
...
function CouponRedemptionsPanel({ coupon }: CouponRedemptionsPanelProps) {
  const { redemptions, isRedemptionsLoading } = useAdminCouponRedemptions({
    couponId: coupon.sId,
    disabled: false,
  });
...
export function CouponsPage() {
  useAdminPageMetadata({ name: "Coupons" });

  const { coupons, canCreateCoupon, isCouponsLoading, mutate } =
    useAdminCoupons();
  const archiveCoupon = useAdminArchiveCoupon();
