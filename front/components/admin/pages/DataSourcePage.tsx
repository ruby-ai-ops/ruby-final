import { SlackWorkflowsTable } from "@app/components/admin/data_sources/slack/workflows_table";
import { ViewDataSourceTable } from "@app/components/admin/data_sources/view";
import { AdminPermissionTree } from "@app/components/admin/AdminConnectorPermissionsTree";
import { SlackChannelPatternInput } from "@app/components/admin/AdminSlackChannelPatternInput";
...
import { useAdminDocuments, useAdminTables } from "@app/admin-app/swr";
import { useAdminPageMetadata } from "@app/admin-app/swr/currentPage";
import { useAdminDataSourceDetails } from "@app/admin-app/swr/data_source_details";
import type { FeaturesType } from "@app/types/api/admin/data_sources";
import type {
...
function FolderDisplay({
  ...
  const {
    ...
    isDocumentsLoading,
    isDocumentsError,
  } = useAdminDocuments(owner, dataSource, limit, offsetDocument);

  const { tables, total: totalTables } = useAdminTables(
    owner,
    dataSource,
    limit,
    offsetTable
...
async function handleCheckOrFindNotionUrl(
  ...
  const res = await clientFetch(`/api/admin/admin`, {
    method: "POST",
    headers: {
  ...
  const { isSubmitting, submit: onToggle } = useSubmitFunction(async () => {
    try {
      const r = await clientFetch(
        `/api/admin/workspaces/${owner.sId}/data_sources/${dataSource.sId}/config`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
...
export function DataSourcePage() {
  ...
  const onDisplayDocumentSource = (
    ...
    if (
      ...
      window.open(
        `/admin/${owner.sId}/data_sources/${dataSourceId}/view?documentId=${encodeURIComponent(documentId)}`
      );
    }
  };
  ...
  return (
    <>
      ...
      {dataSource.connectorProvider === "notion" && (
        <AdminAlert variant="destructive" className="my-4">
          <AdminAlertDescription>
            Please read{" "}
            <a
              href="http://go/admin-notion"
              target="_blank"
            ...
            <Button
              ...
              onClick={() => {
                if (
                  ...
                  void router.push(
                    `/admin/${owner.sId}/data_sources/${dataSource.sId}/search`
                  );
                }
              }}
              label="Search Data"
              ...
              <Button
                ...
                onClick={() => {
                  if (
                    ...
                    void router.push(
                      `/admin/${owner.sId}/data_sources/${dataSource.sId}/query`
                    );
                  }
                }}
                label="Query Data"
                ...
                <Button
                  ...
                  onClick={() => {
                    if (
                      ...
                      void router.push(
                        `/admin/${owner.sId}/data_sources/${dataSource.sId}/notion-requests`
                      );
                    }
                  }}
