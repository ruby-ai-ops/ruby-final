...
import { Input, Spinner } from "@ruby-ai/ui";
...
export function DataSourceSearchPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const searchRes = await clientFetch(
        `/api/admin/workspaces/${owner.sId}/data_sources/${dataSource.sId}/search?` +
  ...
  const onDisplayDocumentSource = (documentId: string) => {
    if (
      ...
      window.open(
        `/admin/${owner.sId}/data_sources/${dataSource.sId}/view?documentId=${encodeURIComponent(documentId)}`
      );
    }
  };
