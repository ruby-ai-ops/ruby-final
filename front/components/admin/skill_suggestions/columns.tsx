...
import { Chip, IconButton, Trash01 } from "@ruby-ai/ui";
...
async function deleteSuggestion(
  ...
  try {
    const r = await clientFetch(
      `/api/admin/workspaces/${owner.sId}/skills/${skillId}/suggestions?suggestionSId=${suggestion.sId}`,
      {
        method: "DELETE",
        headers: {
...
export function makeColumnsForSkillSuggestions(
  ...
  return [
    ...
    },
    {
      accessorKey: "state",
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="State" />
      ),
      cell: ({ row }) => {
    ...
    },
    {
      accessorKey: "source",
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Source" />
      ),
      filterFn: (row, id, value) => {
        return value.includes(row.getValue(id));
      },
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Notification" />
      ),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Analysis" />
      ),
      cell: ({ row }) => {
        return (
          <ClickableCell
    ...
    },
    {
      ...
      header: ({ column }) => (
        <AdminColumnSortableHeader column={column} label="Updated by" />
      ),
      cell: ({ row }) => {
