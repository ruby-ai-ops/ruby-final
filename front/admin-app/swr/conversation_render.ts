...
interface UseAdminRenderConversationProps {
  owner: LightWorkspaceType;
  conversationId: string;
}

export function useAdminRenderConversation({
  owner,
  conversationId,
}: UseAdminRenderConversationProps) {
  ...
  async function renderConversation(
    ...
    try {
      const response = await clientFetch(
        `/api/admin/workspaces/${owner.sId}/conversations/${conversationId}/render`,
        {
          method: "POST",
