...
import type { AdminPlanTypeFilter } from "@app/lib/plans/plan_codes";
import { emptyArray } from "@app/lib/swr/swr";
import { getUniqueCells } from "@app/admin-app/swr/cells";
import type { GetAdminSearchItemsResponseBody } from "@app/types/api/admin/search";
import type {
  GetAdminWorkspacesResponseBody,
  AdminWorkspaceType,
} from "@app/types/api/admin/workspaces";
 ...
 */
export function useAdminSearchAllCells({
  disabled,
  search,
  cells,
}: {
  ...
  const [results, setResults] = useState<AdminItemBase[]>(emptyArray());
    ...
    const run = async () => {
      try {
        const cellPromises = getUniqueCells(cells).map(async (cell) => {
          ...
          const url = `${baseUrl}/api/admin/search?${queryParams.toString()}`;
          ...
          const data: GetAdminSearchItemsResponseBody = await response.json();
        ...
        const allResults: AdminItemBase[] = [];
        let hasErrors = false;
...
export type AdminWorkspaceWithCell = AdminWorkspaceType;

/**
 * Search workspaces across all cells in parallel.
 */
export function useAdminWorkspacesAllCells({
  disabled,
  search,
  upgraded,
  planType,
  ...
  const [workspaces, setWorkspaces] = useState<AdminWorkspaceWithCell[]>(
    emptyArray()
  );
  const [isLoading, setIsLoading] = useState(false);
  const [isError, setIsError] = useState(false);
    ...
    const run = async () => {
      try {
        const cellPromises = cellsToFetch.map(async (cell) => {
          const baseUrl = cell.url;
          const url = `${baseUrl}/api/admin/workspaces?${queryParams.toString()}`;
          ...
          const data: GetAdminWorkspacesResponseBody = await response.json();
          return {
        ...
        const settledResults = await Promise.allSettled(cellPromises);
        const allWorkspaces: AdminWorkspaceWithCell[] = [];
        let hasErrors = false;
