...
} from "@app/lib/api/admin/degraded_models";
...
export function useUpdateAdminDegradedModels() {
  const sendNotification = useSendNotification();

  return useCallback(
    async (updates: DegradedModelEndpointUpdateType[]): Promise<boolean> => {
      try {
        const res = await clientFetch("/api/admin/degraded_models", {
          method: "POST",
