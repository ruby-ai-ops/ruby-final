...
function toAdminConsumptionTopRows(
  ...
  if ("agents" in data) {
    return rows.map((row) => ({
      ...
      detailsHref: row.modelId
        ? `/admin/${workspaceId}/assistants/${row.id}`
        : undefined,
    }));
  }
  if ("groups" in data) {
    return rows.map((row) => ({
      ...
      detailsHref:
        row.name !== row.id
          ? `/admin/${workspaceId}/groups/${row.id}`
          : undefined,
    }));
  }
  if ("skills" in data) {
    return rows.map((row) => ({
      ...
      detailsHref:
        row.name !== row.id
          ? `/admin/${workspaceId}/skills/${row.id}`
          : undefined,
    }));
  }
...
export function useAdminConsumptionOverview({
  workspaceId,
  period,
  filter,
  disabled,
}: UseConsumptionOverviewParams) {
  const url = `/api/admin/workspaces/${workspaceId}/analytics/consumption/overview`;
...
export function useAdminConsumptionFacets({
  workspaceId,
  period,
  filter,
  disabled,
}: UseConsumptionFacetsParams) {
  const url = `/api/admin/workspaces/${workspaceId}/analytics/consumption/facets`;
  const body: ConsumptionBody = {
    period: period.kind,
    days:
      period.kind === "days" ? period.days : DEFAULT_CONSUMPTION_PERIOD_DAYS,
...
export function useAdminConsumptionTimeseries({
  workspaceId,
  period,
  granularity,
  mode,
  ...
  const url = `/api/admin/workspaces/${workspaceId}/analytics/consumption/timeseries`;
...
export function useAdminConsumptionTop({
  workspaceId,
  dimension,
  period,
  limit,
  ...
  const url = `/api/admin/workspaces/${workspaceId}/analytics/consumption/${CONSUMPTION_TOP_ENDPOINTS[dimension]}`;
  ...
  const rows = useMemo(
    () =>
      data
        ? toAdminConsumptionTopRows(data, workspaceId)
