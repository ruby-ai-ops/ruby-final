...
/**
 * Fetch the same admin path from every unique cell in parallel.
 * Absolute URLs bypass the selected-cell base URL rewrite in `clientFetch`.
 */
export async function fetchAdminFromAllCells<T>({
  cells,
  path,
  init,
}: {
  cells: CellInfo[];
  /** Absolute path beginning with `/api/admin/...`, optionally with a query string. */
  path: string;
  init?: RequestInit;
