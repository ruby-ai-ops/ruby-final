...
async function main() {
  ...
  for (const [
    ...
    if (is_notion) {
      ...
      //  -- if the row has `_ruby_id` or `__ruby_id` in `content` -> use that
        ...
        await Promise.all(
          c.map(async ({ oldUniqueIds, newUniqueId }) => {
            ...
            const rowsByRowId = _.groupBy(rows, (r) => {
              ...
              if (content._ruby_id || content.__ruby_id) {
                id = content._ruby_id || content.__ruby_id;
            ...
            for (const [rowId, rowGroup] of Object.entries(rowsByRowId)) {
              ...
              // Create the target `content` object by removing the `_ruby_id` and `__ruby_id` fields
              const parsedContent = JSON.parse(latestRow.content);
              delete parsedContent._ruby_id;
              delete parsedContent.__ruby_id;
