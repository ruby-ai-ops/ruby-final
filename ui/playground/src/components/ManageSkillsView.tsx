...
/** Ruby maintains its own skills, so they are never anyone's to batch-edit. */
function isSkillSelectable(skill: ManagedSkill): boolean {
  return !skill.isRubyProvided && skill.status === "active";
}
  ...
  return [
      ...
      cell: (info: CellContext<RowData, number>) => {
        ...
        return (
          <DataTable.BasicCellContent
            ...
            tooltip={
              usageCount === null
                ? "Ruby skills are always active, so message usage does not apply"
    ...
    },
    {
      ...
      cell: (info: CellContext<RowData, string[]>) => {
        ...
        // A skill with no editors is one Ruby maintains, which the table says
        // by naming Ruby rather than leaving the cell blank.
        if (editors.length === 0) {
          return <DataTable.BasicCellContent label="Ruby" />;
        }
