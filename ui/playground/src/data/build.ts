...
/** How insistently Ruby asks before running a tool. */
...
export type MockTool = {
   ...
   * Ruby at. A built-in tool has none, and the product then offers neither the
...
export type ManagedSkill = {
  ...
  /** Messages over the last 30 days. A Ruby skill is always on, so it has none. */
  usageCount: number | null;
  usedByAgentIds: string[];
  editorIds: string[];
  updatedAt: Date;
  /** A Ruby-provided skill has no editors and can never be batch-edited. */
  isRubyProvided: boolean;
  guidelines: string;
  toolIds: string[];
  spaceIds: string[];
};
...
function decorateSkill(
  ...
  const isRubyProvided = skillSeed.isRubyProvided ?? false;
...
> = {
  ...
  global: {
    label: "Default",
    color: "primary",
    tooltip: "A default agent provided by Ruby.",
  },
};
...
/**
 * The default agents Ruby ships. They sit in their own tab, cannot be edited,
 * and so are never selectable.
 */
const GLOBAL_AGENT_SEEDS: Agent[] = [
  {
    id: "agent-global-ruby",
    name: "ruby",
    emoji: "✨",
